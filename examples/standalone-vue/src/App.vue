...
const standaloneSelect=ref('east')
const standaloneRemoteSelect=ref('')
...
const standaloneCollapseOpen=ref(['contract'])
const standaloneCollapseState=ref('ready')
const standaloneCollapseItems=[{key:'contract',label:'Public contract',content:'Props, emits, slots and imperative methods are available from both root and component subpath imports.',extra:'Required'},{key:'keyboard',label:'Keyboard and ARIA',content:'Arrow, Home and End move between enabled heading buttons while each trigger remains linked to its Region.'},{key:'lifecycle',label:'Consumer lifecycle',content:'Lazy mount, destroy-on-hide and asynchronous guards are consumer-controlled.'},{key:'locked',label:'Restricted section',content:'Disabled sections are skipped.',disabled:true}]
async function guardStandaloneCollapse(item,open){if(item.key==='lifecycle'&&open){standaloneCollapseState.value='checking policy';await new Promise(resolve=>setTimeout(resolve,260))}return true}
...
<template>
  <main class="standalone-shell">
    ...
    <UiCard data-breadcrumb-state-contract title="Consumer navigation path" subtitle="Semantic, collapsible and typed UiBreadcrumb from the packed dependency">
      ...
      <template #footer><UiTag color="blue" variant="outlined">{{ standaloneBreadcrumbExpanded?'Expanded path':'Collapsed path' }}</UiTag></template>
    </UiCard>
    ...
    <UiCard data-select-state-contract title="Consumer production selects" subtitle="Controlled state, native forms, remote search and complete keyboard contracts from the packed dependency">
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:16px">
        <UiFormItem label="Delivery cluster" required help="Field mapping · description / keyword search · native reset"><UiSelect v-model="standaloneSelect" :options="standaloneSelectOptions" :field-names="{label:'name',value:'id',description:'detail',keywords:'terms',disabled:'locked'}" searchable clearable name="deliveryCluster" required @change="(value,meta)=>standaloneSelectState=`${value} · ${meta.source}`"><template #prefix>◈</template></UiSelect></UiFormItem>
        <UiFormItem label="Remote release lane" help="Debounce · AbortSignal · stale-response guard · cache"><UiSelect v-model="standaloneRemoteSelect" searchable clearable :remote-method="fetchStandaloneSelect" :remote-debounce="180" :remote-min-chars="1" placeholder="Search package lane" @change="(value,meta)=>standaloneSelectState=`remote ${value} · ${meta.source}`"/></UiFormItem>
    ...
    <UiCard data-tag-state-contract title="Consumer tag vocabulary" subtitle="Interactive UiTag contract from the packed dependency">
    ...
    <UiCard data-tooltip-state-contract title="Consumer tooltip guidance" subtitle="Typed triggers, controlled state and accessible descriptions from the packed dependency">
      <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
        <UiTooltip v-model:open="standaloneTooltipOpen" trigger="click" placement="bottom-start" wrap :max-width="240" content="This controlled package tooltip closes on Escape, outside pointer actions, or a second trigger click."><UiButton variant="outline">Release guidance</UiButton></UiTooltip>
        <UiTooltip content="Hover and keyboard focus share independent open reasons." :show-delay="120" :hide-delay="80"><UiButton variant="text">Keyboard help</UiButton></UiTooltip>
        <UiTag color="blue">{{ standaloneTooltipOpen?'tooltip open':'tooltip closed' }}</UiTag>
      </div>
    </UiCard>

    <UiCard data-popover-state-contract title="Consumer popover actions" subtitle="Typed interactive overlays, focus policy and dismissal from the packed dependency">
      <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
        <UiPopover v-model="standalonePopoverOpen" title="Release actions" trigger="click" placement="bottom-start" :width="280" auto-focus trap-focus>
          <template #trigger><UiButton variant="outline">Review release</UiButton></template>
          <p style="margin:0">Inspect the package, verification record and rollback before publishing.</p>
          <template #footer="{close}"><UiButton size="sm" variant="text" data-popover-keep-open>Keep open</UiButton><UiButton size="sm" @click="close('content')">Approve</UiButton></template>
        </UiPopover>
        <UiPopover trigger="hover focus" title="Operational note" :show-delay="120" :hide-delay="80"><template #trigger><UiButton variant="text">Hover + focus</UiButton></template>Trigger and panel preserve independent interaction reasons.</UiPopover>
        <UiTag color="blue">{{ standalonePopoverOpen?'popover open':'popover closed' }}</UiTag>
      </div>
    </UiCard>
    ...
    <UiCard data-collapse-state-contract title="Consumer disclosure sections" subtitle="Controlled state, lifecycle policy and accessible header navigation from the packed dependency">
      <UiCollapse v-model="standaloneCollapseOpen" :items="standaloneCollapseItems" lazy loop :before-toggle="guardStandaloneCollapse" aria-label="Consumer release sections" @change="(_value,meta)=>standaloneCollapseState=`${meta.open?'open':'close'}:${meta.key}:${meta.source}`"><template #item-lifecycle="{content}"><div style="display:flex;align-items:flex-start;gap:10px"><UiTag color="green">Verified</UiTag><span>{{ content }}</span></div></template></UiCollapse>
      <template #footer><UiTag color="blue">{{ standaloneCollapseOpen.join(' · ')||'all closed' }}</UiTag><span style="margin-inline-start:10px;color:var(--text-secondary)">{{ standaloneCollapseState }}</span></template>
    </UiCard>
