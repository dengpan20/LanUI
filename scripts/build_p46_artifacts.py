...
BASELINE = ROOT / ".baseline" / "maturity-p46-v1"
OLD_ARCHIVE = BASELINE / "package-original.zip"
NEW_ARCHIVE = ARTIFACTS / "lan-ui-v1.zip"
PATCH = ARTIFACTS / "maturity-p46-v1.patch"
BASELINE_ARCHIVE = ARTIFACTS / "maturity-p46-v1-baseline.zip"
ROLLBACK = ARTIFACTS / "rollback-maturity-p46-v1.ps1"


def rollback_script(added: list[str], baseline_count: int, artifact_count: int) -> str:
    script = base.rollback_script(added, baseline_count, artifact_count).replace("p11", "p46").replace("P11", "P46")
    return script.replace(
        "    'rollback-maturity-p46-v1.ps1'",
        "    'rollback-maturity-p46-v1.ps1',\n    'lan-ui-design-system-1.42.0.tgz',\n    'lan-ui-design-system-1.42.0.tgz.sha256'",
    )


def main() -> None:
    if not OLD_ARCHIVE.is_file() or not NEW_ARCHIVE.is_file():
        raise FileNotFoundError("P46 baseline and modified package archives are required")
    ...
    with tempfile.TemporaryDirectory(prefix="lan-ui-p46-patch-") as folder:
        repo = Path(folder)
        ...
        # Install the binary attribute before the baseline is indexed. Some
        # historical source files are CRLF in the frozen archive and LF in the
        # current workspace; indexing first would normalize those bytes and
        # silently omit the line-ending-only changes from the replay patch.
        (repo / ".git" / "info" / "attributes").write_text("* binary\n", encoding="utf-8")
        base.run_git(git, repo, "add", "-A")
        base.run_git(git, repo, "commit", "-qm", "P46 baseline")
        # Artifact replay is byte-exact across mixed historical line endings.
        # Force binary records without changing the archived workspace attributes.
    ...
    print(f"P46_ARTIFACTS PASS added={len(added)} changed={len(changed)} deleted={len(deleted)} baseline={len(old_files)} artifacts={len(originals)} patch={PATCH.stat().st_size}B")
