...
const required=[
  ...
  'dist-lib/styles/core.css','dist-lib/styles/UiAffix.css','dist-lib/styles/UiAnchor.css','dist-lib/styles/UiTour.css','dist-lib/styles/UiWatermark.css','dist-lib/components/UiAffix.js','dist-lib/components/UiAffix.d.ts','dist-lib/components/UiAnchor.js','dist-lib/components/UiAnchor.d.ts','dist-lib/components/UiTour.js','dist-lib/components/UiTour.d.ts','dist-lib/components/UiWatermark.js','dist-lib/components/UiWatermark.d.ts',
]
...
runPnpm(['install','--ignore-workspace','--offline','--ignore-scripts','--frozen-lockfile'],{cwd:consumerRoot})
  ...
  writeFileSync(join(consumerRoot,'ssr.mjs'),`import { createSSRApp, h } from 'vue'\nimport { renderToString } from 'vue/server-renderer'\nimport { UiAffix, UiAnchor } from 'lan-ui-design-system'\nimport UiButton from 'lan-ui-design-system/components/UiButton'\nimport UiTour from 'lan-ui-design-system/components/UiTour'\nimport UiWatermark from 'lan-ui-design-system/components/UiWatermark'\nconst app=createSSRApp({render:()=>h('main',[h(UiAffix,{offset:8},()=> 'Packed sticky action'),h(UiAnchor,{items:[{title:'Overview',href:'#overview'}],affix:false}),h(UiButton,null,{default:()=> 'Packed action'}),h(UiWatermark,{content:['Lan UI','PACKED'],ariaLabel:'Packed watermark'},{default:()=>h('article','Packed protected document')}),h(UiTour,{modelValue:true,ariaLabel:'Packed tour',steps:[{title:'Packed onboarding',description:'SSR tour'}]})])})\nconst context={}\nconst html=await renderToString(app,context)\nif(!html.includes('ui-affix')||!html.includes('Packed sticky action')||!html.includes('ui-anchor')||!html.includes('Packed action')||!html.includes('Packed protected document')||!html.includes('Packed watermark')||!context.teleports?.body?.includes('Packed onboarding'))throw new Error('SSR output mismatch')\nconsole.log('PACKED_SSR PASS affix=true anchor=true button=true tour=true watermark=true')\n`)
...
assert(componentNames.length===73,'Public component count mismatch')
