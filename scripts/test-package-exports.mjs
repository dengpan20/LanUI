...
if (components.length !== 89) throw new Error(`Expected 89 public components, found ${components.length}`)
...
const motion = await import('lan-ui-design-system/motion')
...
for(const name of ['normalizeMotionPreference','resolveMotionPreference','motionPreferenceToStyle','createMotionController','useReducedMotion'])if(typeof motion[name]!=='function'||main[name]!==motion[name])throw new Error(`Motion utility export parity failed: ${name}`)
if(motion.MOTION_PREFERENCES.join(',')!=='full,reduced,system'||motion.resolveMotionPreference('system',true)!=='reduced'||motion.motionPreferenceToStyle('reduced')['--motion-time']!=='.01ms')throw new Error('Built motion preference contract failed')
const packageMotionController=motion.createMotionController({preference:'system',storage:null,matchMedia:null})
if(packageMotionController.mount(null).resolvedPreference!=='full'||packageMotionController.reduced)throw new Error('Built motion SSR controller contract failed')
packageMotionController.dispose()
...
console.log(`PACKAGE_EXPORTS PASS components=${components.length} js=${jsEntries.length} types=${typeEntries.length} styles=${styleEntries.length} contracts=props+emits+slots named-default-parity=pass ssr=pass feedback-isolation=pass locale-registry=lazy+chain+activation color=parser+contrast date-adapter=iana+dst icons=isolated+sanitized theme=presets+controller motion=system+controller subpaths=color,config,date,feedback,icons,motion,plugin,theme`)
