...
BASELINE = ROOT / ".baseline" / "maturity-p8-v1"
OLD_ARCHIVE = BASELINE / "package-original.zip"
NEW_ARCHIVE = ARTIFACTS / "lan-ui-v1.zip"
PATCH = ARTIFACTS / "maturity-p8-v1.patch"
BASELINE_ARCHIVE = ARTIFACTS / "maturity-p8-v1-baseline.zip"
ROLLBACK = ARTIFACTS / "rollback-maturity-p8-v1.ps1"
...
def rollback_script(added: list[str], baseline_count: int) -> str:
    p8_roles = [
        "maturity-p8-v1-baseline.zip", "maturity-p8-v1.patch",
        "maturity-p8-v1-verification.md", "maturity-p8-v1.sha256",
        "rollback-maturity-p8-v1.ps1",
    ]
    return f"""param(
...
$archive = Join-Path $PSScriptRoot 'maturity-p8-v1-baseline.zip'
if (-not (Test-Path -LiteralPath $archive -PathType Leaf)) {{ throw "Missing baseline archive: $archive" }}
$temp = Join-Path ([IO.Path]::GetTempPath()) ("lan-ui-rollback-p8-" + [guid]::NewGuid().ToString('N'))
...
{ps_array(added)}
  ...
  $artifactRestore = [ordered]@{{
    ...
    'maturity-p7-v1.patch.orig' = 'maturity-p7-v1.patch'
    'maturity-p7-v1-verification.md.orig' = 'maturity-p7-v1-verification.md'
    'maturity-p7-v1.sha256.orig' = 'maturity-p7-v1.sha256'
    'maturity-p7-v1-baseline.zip.orig' = 'maturity-p7-v1-baseline.zip'
    'rollback-maturity-p7-v1.ps1.orig' = 'rollback-maturity-p7-v1.ps1'
  }}
...
def main() -> None:
    if not OLD_ARCHIVE.is_file() or not NEW_ARCHIVE.is_file():
        raise FileNotFoundError("P8 baseline and modified package archives are required")
    ...
    git = git_executable()
    with tempfile.TemporaryDirectory(prefix="lan-ui-p8-patch-") as folder:
        repo = Path(folder)
        ...
        run_git(git, repo, "commit", "-qm", "P8 baseline")
    ...
    with zipfile.ZipFile(BASELINE_ARCHIVE, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        ...
        for name in ["lan-ui-v1.patch.orig", "maturity-p7-v1.patch.orig", "maturity-p7-v1-verification.md.orig", "maturity-p7-v1.sha256.orig", "maturity-p7-v1-baseline.zip.orig", "rollback-maturity-p7-v1.ps1.orig"]:
    ...
    print(f"P8_ARTIFACTS PASS added={len(added)} changed={len(changed)} deleted={len(deleted)} baseline={len(old_files)} patch={PATCH.stat().st_size}B")
