...
const required={
  ...
  'UiQRCode.vue':['qrcode-generator','shape-rendering="crispEdges"','aria-labelledby','data-status','qrCode.status.','URL.createObjectURL','defineExpose'],
  'UiBarcode.vue':['jsbarcode','shape-rendering="crispEdges"','aria-labelledby','data-format','barcode.status.','URL.createObjectURL','defineExpose'],
  'UiCronEditor.vue':['parseCronExpression','getNextCronRuns','data-valid','cron.error.','aria-invalid','defineExpose'],
  'UiKeyValueEditor.vue':['validationFor','importText','data-valid','keyValue.error.','aria-invalid','uiFormItemContext','defineExpose'],
...
const styleManifest=JSON.parse(fs.readFileSync('style-manifest.json','utf8'));if(styleManifest.components?.length!==88)failures.push(`style-manifest:components:${styleManifest.components?.length}`);if(!packageJson.exports?.['./styles/*.css']||!packageJson.exports?.['./style-manifest'])failures.push('package-export:component-styles')
