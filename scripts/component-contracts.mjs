...
const required={
  'UiBreadcrumb.vue':['data-ui-breadcrumb','ui-breadcrumb-overflow','currentKey','maxItems','defaultExpanded','update:expanded','focusItem','defineExpose'],
  ...
  'UiTable.vue':['data-ui-table','defaultSelectedRows','defaultExpandedRows','selectionMode','isRowSelectable','beforeSelect','beforeExpand','defaultSortKey','sortOrders','defaultFilters','filterOpenKey','defaultColumnWidths','columnWidths','column-resize-start','virtualRange','aria-rowcount','aria-sort','role="separator"','ArrowDown','useDirection','import UiCheckbox','size="sm"','defineExpose'],
  ...
  'UiTreeSelect.vue':['data-ui-tree-select','defaultValue','update:open','update:expandedKeys','fieldNames','loadData','AbortController','load-error','checkStrictly','maxCount','minCount','aria-multiselectable','aria-activedescendant','ArrowRight','ArrowLeft','ui-tree-select-native','onFormReset','useTeleportThemeScope','useFloatingPosition','defineExpose'],
  ...
  'UiFloatButton.vue':['data-ui-float-button','visible-change','back-top','scrollTarget','aria-busy','UiTooltip','useReducedMotion','defineExpose'],
  'UiFloatButtonGroup.vue':['data-ui-float-button-group','beforeOpenChange','closeOnEscape','provide','register','ArrowDown','Home','focusFirst','aria-controls','guard-error','defineExpose'],
}
...
const rangeBase=apiManifest.components?.find(component=>component.name==='UiDateRangePicker');for(const wrapperName of ['UiTimeRangePicker','UiDateTimeRangePicker']){const wrapper=apiManifest.components?.find(component=>component.name===wrapperName);if(!rangeBase||!wrapper||JSON.stringify(wrapper.emits)!==JSON.stringify(rangeBase.emits))failures.push(`range-wrapper-emits-parity:${wrapperName}`);if(!rangeBase||!wrapper||JSON.stringify(wrapper.slots)!==JSON.stringify(rangeBase.slots))failures.push(`range-wrapper-slots-parity:${wrapperName}`);if(!declarations.includes(`export interface ${wrapperName}Emits extends UiDateRangePickerEmits`))failures.push(`range-wrapper-emits-declaration:${wrapperName}`);if(!declarations.includes(`export interface ${wrapperName}Slots extends UiDateRangePickerSlots`))failures.push(`range-wrapper-slots-declaration:${wrapperName}`)}
const styleManifest=JSON.parse(fs.readFileSync('style-manifest.json','utf8'));if(styleManifest.components?.length!==92)failures.push(`style-manifest:components:${styleManifest.components?.length}`);if(!packageJson.exports?.['./styles/*.css']||!packageJson.exports?.['./style-manifest'])failures.push('package-export:component-styles')
