...
const apiManifest=JSON.parse(fs.readFileSync('api-manifest.json','utf8'));if(apiManifest.schemaVersion!==2||apiManifest.components?.length!==62)failures.push(`api-manifest:schema-components:${apiManifest.schemaVersion}:${apiManifest.components?.length}`);for(const component of apiManifest.components||[])if(component.runtimeExports?.length!==2||!component.runtimeExports.includes('default')||!component.runtimeExports.includes(component.name)||!component.props?.length||!Array.isArray(component.emits)||!Array.isArray(component.slots)||component.emitsType!==`${component.name}Emits`||component.slotsType!==`${component.name}Slots`)failures.push(`api-manifest:parity:${component.name}`)
for(const file of ['UiTabs.vue','UiSegmented.vue','UiMenu.vue','UiTreeSelect.vue','UiCascader.vue','UiTable.vue','UiCommandPalette.vue','UiColorPicker.vue'])if(!fs.readFileSync(`src/components/${file}`,'utf8').includes('useDirection'))failures.push(`rtl:${file}`)
const styleManifest=JSON.parse(fs.readFileSync('style-manifest.json','utf8'));if(styleManifest.components?.length!==62)failures.push(`style-manifest:components:${styleManifest.components?.length}`);if(!packageJson.exports?.['./styles/*.css']||!packageJson.exports?.['./style-manifest'])failures.push('package-export:component-styles')
