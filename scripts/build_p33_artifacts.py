...
BASELINE = ROOT / ".baseline" / "maturity-p33-v1"
OLD_ARCHIVE = BASELINE / "package-original.zip"
NEW_ARCHIVE = ARTIFACTS / "lan-ui-v1.zip"
PATCH = ARTIFACTS / "maturity-p33-v1.patch"
BASELINE_ARCHIVE = ARTIFACTS / "maturity-p33-v1-baseline.zip"
ROLLBACK = ARTIFACTS / "rollback-maturity-p33-v1.ps1"


def rollback_script(added: list[str], baseline_count: int, artifact_count: int) -> str:
    return base.rollback_script(added, baseline_count, artifact_count).replace("p11", "p33").replace("P11", "P33")


def main() -> None:
    if not OLD_ARCHIVE.is_file() or not NEW_ARCHIVE.is_file():
        raise FileNotFoundError("P33 baseline and modified package archives are required")
    ...
    with tempfile.TemporaryDirectory(prefix="lan-ui-p33-patch-") as folder:
        repo = Path(folder)
        ...
        base.run_git(git, repo, "commit", "-qm", "P33 baseline")
    ...
    print(f"P33_ARTIFACTS PASS added={len(added)} changed={len(changed)} deleted={len(deleted)} baseline={len(old_files)} artifacts={len(originals)} patch={PATCH.stat().st_size}B")
