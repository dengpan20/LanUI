...
BASELINE = ROOT / ".baseline" / "maturity-p11-v1"
OLD_ARCHIVE = BASELINE / "package-original.zip"
NEW_ARCHIVE = ARTIFACTS / "lan-ui-v1.zip"
PATCH = ARTIFACTS / "maturity-p11-v1.patch"
BASELINE_ARCHIVE = ARTIFACTS / "maturity-p11-v1-baseline.zip"
ROLLBACK = ARTIFACTS / "rollback-maturity-p11-v1.ps1"
...
def rollback_script(added: list[str], baseline_count: int, artifact_count: int) -> str:
    p11_roles = [
        "maturity-p11-v1-baseline.zip", "maturity-p11-v1.patch",
        "maturity-p11-v1-verification.md", "maturity-p11-v1.sha256",
        "rollback-maturity-p11-v1.ps1",
    ]
    return f"""param(
...
if (-not (Test-Path -LiteralPath (Join-Path $root 'package.json') -PathType Leaf)) {{ throw "TargetRoot is not a Lan UI workspace: $root" }}
$archive = Join-Path $PSScriptRoot 'maturity-p11-v1-baseline.zip'
if (-not (Test-Path -LiteralPath $archive -PathType Leaf)) {{ throw "Missing baseline archive: $archive" }}
$temp = Join-Path ([IO.Path]::GetTempPath()) ("lan-ui-rollback-p11-" + [guid]::NewGuid().ToString('N'))
...
{ps_array(added)}
  ...
  foreach ($original in Get-ChildItem -LiteralPath (Join-Path $temp 'artifacts') -File -Filter '*.orig') {{
    $targetName = $original.Name.Substring(0, $original.Name.Length - 5)
    Copy-Item -LiteralPath $original.FullName -Destination (Join-Path $targetArtifacts $targetName) -Force
  }}
  $mismatches = @()
  foreach ($file in $baselineFiles) {{
    $relative = $file.FullName.Substring($workspace.Length).TrimStart('\\', '/')
...
{ps_array(p11_roles)}
  ...
  foreach ($relative in $p11Roles) {{
    $target = Join-Path $targetArtifacts $relative
    if (Test-Path -LiteralPath $target -PathType Leaf) {{ Remove-Item -LiteralPath $target -Force }}
  }}
  Write-Output "ROLLBACK PASS restored={baseline_count} removed=$($added.Count) generated=dist,dist-lib artifacts={artifact_count} target=$root"
}}
finally {{
...
def main() -> None:
    if not OLD_ARCHIVE.is_file() or not NEW_ARCHIVE.is_file():
        raise FileNotFoundError("P11 baseline and modified package archives are required")
    ...
    git = git_executable()
    with tempfile.TemporaryDirectory(prefix="lan-ui-p11-patch-") as folder:
        repo = Path(folder)
        ...
        run_git(git, repo, "commit", "-qm", "P11 baseline")
        for child in repo.iterdir():
            if child.name != ".git":
                shutil.rmtree(child) if child.is_dir() else child.unlink()
        write_tree(repo, new_files)
        run_git(git, repo, "add", "-A")
        diff = run_git(git, repo, "diff", "--cached", "--binary", "--full-index", "--no-ext-diff", "--src-prefix=a/", "--dst-prefix=b/", capture=True).stdout
    ...
    print(f"P11_ARTIFACTS PASS added={len(added)} changed={len(changed)} deleted={len(deleted)} baseline={len(old_files)} artifacts={len(originals)} patch={PATCH.stat().st_size}B")
