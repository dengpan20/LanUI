...
BASELINE = ROOT / ".baseline" / "maturity-p10-v1"
OLD_ARCHIVE = BASELINE / "package-original.zip"
NEW_ARCHIVE = ARTIFACTS / "lan-ui-v1.zip"
PATCH = ARTIFACTS / "maturity-p10-v1.patch"
BASELINE_ARCHIVE = ARTIFACTS / "maturity-p10-v1-baseline.zip"
ROLLBACK = ARTIFACTS / "rollback-maturity-p10-v1.ps1"
...
def rollback_script(added: list[str], baseline_count: int, artifact_count: int) -> str:
    p10_roles = [
        "maturity-p10-v1-baseline.zip", "maturity-p10-v1.patch",
        "maturity-p10-v1-verification.md", "maturity-p10-v1.sha256",
        "rollback-maturity-p10-v1.ps1",
    ]
    return f"""param(
...
$archive = Join-Path $PSScriptRoot 'maturity-p10-v1-baseline.zip'
if (-not (Test-Path -LiteralPath $archive -PathType Leaf)) {{ throw "Missing baseline archive: $archive" }}
$temp = Join-Path ([IO.Path]::GetTempPath()) ("lan-ui-rollback-p10-" + [guid]::NewGuid().ToString('N'))
...
def main() -> None:
    if not OLD_ARCHIVE.is_file() or not NEW_ARCHIVE.is_file():
        raise FileNotFoundError("P10 baseline and modified package archives are required")
    ...
    git = git_executable()
    with tempfile.TemporaryDirectory(prefix="lan-ui-p10-patch-") as folder:
        repo = Path(folder)
        ...
        run_git(git, repo, "commit", "-qm", "P10 baseline")
    ...
    print(f"P10_ARTIFACTS PASS added={len(added)} changed={len(changed)} deleted={len(deleted)} baseline={len(old_files)} artifacts={len(originals)} patch={PATCH.stat().st_size}B")
