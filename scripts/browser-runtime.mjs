...
const UNSAFE_BROWSER_PORTS=new Set([1,7,9,11,13,15,17,19,20,21,22,23,25,37,42,43,53,77,79,87,95,101,102,103,104,109,110,111,113,115,117,119,123,135,139,143,179,389,427,465,512,513,514,515,526,530,531,532,540,556,563,587,601,636,993,995,2049,3659,4045,5060,5061,6000,6566,6665,6666,6667,6668,6669,6697,10080])
export function isUnsafeBrowserPort(port){
  const value=Number(port)
  return UNSAFE_BROWSER_PORTS.has(value)
}
...
export async function startFixtureServer(root){
  ...
  try{
    for(let attempt=1;attempt<=8;attempt+=1){
      ...
      console.log(`BROWSER_SERVER_RETRY_UNSAFE_PORT port=${port} attempt=${attempt}/8`)
      await withBrowserTimeout(()=>server.close(),stageTimeout,'server-close-unsafe-port')
      server=null
    }
    throw new Error('BROWSER_SERVER_UNSAFE_PORTS exhausted safe candidates')
  }catch(error){
...
export async function startStaticFixtureServer(root,{entry='interaction-regression.html',outputDirName='interaction-fixture-dist'}={}){
  ...
  try{
    ...
    for(let attempt=1;attempt<=8;attempt+=1){
      ...
      if(!isUnsafeBrowserPort(port))break
      console.log(`BROWSER_STATIC_SERVER_RETRY_UNSAFE_PORT port=${port} attempt=${attempt}/8`)
      await closeNodeHttpServer(httpServer,{timeout:stageTimeout})
      httpServer=null
    }
    if(!httpServer||isUnsafeBrowserPort(port))throw new Error('BROWSER_STATIC_SERVER_UNSAFE_PORTS exhausted safe candidates')
