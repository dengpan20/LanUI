...
if any(marker not in preview for marker in ["V1.59.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.59.0" or package.get("private") is not False or package.get("license") != "MIT":
...
if distribution_budgets != {"packedFiles": 402, "packedTarballRaw": 500000, "packedUnpackedRaw": 2860000}:
    failures.append("p39:distribution-budgets")
if "Breadcrumb P63" not in components_page or "1.59.0" not in components_page or "V1.59.0" not in preview:
...
if "V1.59.0" not in app or "V1.59.0" not in login_page or "badge:89" not in components_page or "<td>Watermark</td>" not in components_page:
...
if len(visual_baselines) < 36 or any(path.stat().st_size < 1000 for path in visual_baselines):
...
if performance_budgets.get("releaseBaseline", {}).get("enhancementAllowance", {}) != {"packageJsRaw": 32000, "packageJsGzip": 9800, "packageCssRaw": 49600, "packageCssGzip": 8800, "rootCssRaw": 24500, "rootCssGzip": 3500, "subpathConsumerCssRaw": 100, "standaloneExampleJsRaw": 33000, "standaloneExampleCssRaw": 25000, "subpathConsumerJsRaw": 300}:
    failures.append("performance:p63-bounded-component-enhancement-policy")
