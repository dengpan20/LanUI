...
if any(marker not in preview for marker in ["V1.58.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.58.0" or package.get("private") is not False or package.get("license") != "MIT":
...
if distribution_budgets != {"packedFiles": 402, "packedTarballRaw": 489000, "packedUnpackedRaw": 2805000}:
    failures.append("p39:distribution-budgets")
if "Steps P62" not in components_page or "1.58.0" not in components_page or "V1.58.0" not in preview:
...
if "V1.58.0" not in app or "V1.58.0" not in login_page or "badge:89" not in components_page or "<td>Watermark</td>" not in components_page:
...
if len(visual_baselines) < 35 or any(path.stat().st_size < 1000 for path in visual_baselines):
...
if performance_budgets.get("releaseBaseline", {}).get("enhancementAllowance", {}) != {"packageJsRaw": 23000, "packageJsGzip": 7100, "packageCssRaw": 41000, "packageCssGzip": 7300, "rootCssRaw": 19500, "rootCssGzip": 2900, "subpathConsumerCssRaw": 100, "standaloneExampleJsRaw": 24200, "standaloneExampleCssRaw": 20000, "subpathConsumerJsRaw": 100}:
    failures.append("performance:p62-bounded-component-enhancement-policy")
