...
required = [
    ...
    "src/components/UiTreeSelect.vue", "src/components/UiCascader.vue", "src/components/UiTransfer.vue",
...
if api_manifest.get("schemaVersion") != 2 or len(api_manifest.get("root", {}).get("typeExports", [])) < 275 or len(manifest_components) != 57:
...
if len(style_components) != 57 or any(not item.get("bytes") or not item.get("rules") for item in style_components):
...
completion_components = ["UiLayout","UiGrid","UiCol","UiSpace","UiDivider","UiTabs","UiBreadcrumb","UiAvatar","UiBadge","UiSkeleton","UiEmpty","UiAlert","UiDropdown","UiProgress","UiSteps","UiTimeline","UiForm","UiMultiSelect","UiTreeSelect","UiCascader","UiTransfer"]
...
for component in ["UiCheckbox", "UiRadio", "UiSwitch", "UiTooltip", "UiPopover", "UiPopconfirm", "UiMenu", "UiCollapse", "UiDescriptions", "UiResult", "UiSpin", "UiSegmented", "UiConfigProvider", "UiDateRangePicker", "UiTimePicker", "UiIcon", "UiNumberInput", "UiSlider", "UiAutoComplete"]:
...
for component in ["UiAlert", "UiAutoComplete", "UiButton", "UiIcon", "UiInput", "UiNumberInput", "UiSlider", "UiSelect", "UiSteps", "UiTable", "UiSegmented", "UiDescriptions", "UiToastHost", "UiConfigProvider", "UiDateRangePicker", "toast.success"]:
...
direction_sources = "\n".join((ROOT / rel).read_text(encoding="utf-8") for rel in [
    ...
    "src/components/UiTreeSelect.vue", "src/components/UiCascader.vue", "src/components/UiDrawer.vue", "src/components/UiTable.vue",
])
...
for marker in ["axe.run", "wcag22aa", "best-practice", "violations", "incomplete", "autocomplete-open", "multi-select-open", "tree-select-open", "cascader-open", "modal-open", "drawer-rtl-open"]:
    if marker not in accessibility_script:
        failures.append(f"accessibility:browser:{marker}")
if accessibility_script.count("{name:") < 10:
...
if dist_lib.exists():
    ...
    if len(component_js) != 57 or len(component_types) != 57:
    ...
    if len(style_files) != 57 or not (dist_lib / "styles/core.css").is_file() or not (dist_lib / "styles/manifest.json").is_file():
