...
required = [
    ...
    "src/components/UiDatePicker.vue", "src/components/UiDateTimePicker.vue", "src/components/UiDateTimeRangePicker.vue", "src/components/UiTimePicker.vue", "src/components/UiTimeRangePicker.vue", "src/components/UiQRCode.vue", "src/components/UiPagination.vue",
...
if api_manifest.get("schemaVersion") != 3 or len(api_manifest.get("root", {}).get("typeExports", [])) < 697 or len(manifest_components) != 85:
...
if len(api_categories) != 6 or len(api_components) != 85 or sum(item.get("count", 0) for item in api_categories) != 85 or len({item.get("name") for item in api_components}) != 85:
...
if len(style_components) != 85 or any(not item.get("bytes") or not item.get("rules") for item in style_components):
...
if any(marker not in preview for marker in ["V1.50.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.50.0" or package.get("private") is not False or package.get("license") != "MIT":
...
for marker in ["--ignore-workspace", "--offline", "packedManifest.private===false", "renderToString", "typescript/bin/tsc", "internals=absent", "componentNames.length===85", "PACKED_CAROUSEL_SSR PASS", "PACKED_TIME_RANGE_SSR PASS", "PACKED_DATE_TIME_SSR PASS", "PACKED_QR_CODE_SSR PASS", "allowedTopLevel", "reproducible=pass", "allowList=pass"]:
...
if distribution_budgets != {"packedFiles": 386, "packedTarballRaw": 430000, "packedUnpackedRaw": 2430000}:
    failures.append("p39:distribution-budgets")
if "QR Code P54" not in components_page or "1.50.0" not in components_page or "V1.50.0" not in preview:
...
for marker in ["RELEASE_CONTRACT PASS", "componentNames.length===85", "actions/upload-artifact@v7", "actions/attest@v4", "artifact-metadata: write", "gh release create"]:
...
if "V1.50.0" not in app or "V1.50.0" not in login_page or "badge:85" not in components_page or "<td>Watermark</td>" not in components_page:
...
if any(marker not in components_page for marker in ["UiQRCode", "QR Code P54", "data-qr-code-state-contract", "<td>QRCode</td>"]):
...
if performance_budgets.get("releaseBaseline", {}).get("componentCount") != 69 or performance_budgets.get("releaseBaseline", {}).get("perComponentAllowance", {}) != {"packageJsRaw": 6000, "packageJsGzip": 3375, "packageCssRaw": 5750, "packageCssGzip": 875, "largestChunkRaw": 225, "largestChunkGzip": 180, "largestComponentCssRaw": 1500, "largestComponentCssGzip": 150, "standaloneExampleJsRaw": 6250}:
    failures.append("performance:p50-additive-component-policy")
...
if dist_lib.exists():
    ...
    if len(component_js) != 85 or len(component_types) != 85:
    ...
    if len(style_files) != 85 or not (dist_lib / "styles/core.css").is_file() or not (dist_lib / "styles/manifest.json").is_file():
