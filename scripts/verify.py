...
required = [
    ...
    "LICENSE", "scripts/packed-consumer-regression.mjs", "scripts/build_p39_artifacts.py", "scripts/build_p41_artifacts.py", "scripts/build_p42_artifacts.py",
    ...
    "src/components/UiNotification.vue", "src/components/UiTour.vue", "src/components/UiWatermark.vue", "src/components/UiFormItem.vue", "src/components/UiFormList.vue", "src/components/UiSchemaForm.vue",
...
if api_manifest.get("schemaVersion") != 3 or len(api_manifest.get("root", {}).get("typeExports", [])) < 480 or len(manifest_components) != 72:
...
if len(api_categories) != 6 or len(api_components) != 72 or sum(item.get("count", 0) for item in api_categories) != 72 or len({item.get("name") for item in api_components}) != 72:
...
if len(style_components) != 72 or any(not item.get("bytes") or not item.get("rules") for item in style_components):
...
if any(marker not in preview for marker in ["V1.38.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.38.0" or package.get("private") is not False or package.get("license") != "MIT":
...
for marker in ["--ignore-workspace", "--offline", "packedManifest.private===false", "renderToString", "typescript/bin/tsc", "internals=absent", "componentNames.length===72", "allowedTopLevel", "reproducible=pass", "allowList=pass"]:
...
if "Watermark P42" not in components_page or "1.38.0" not in components_page or "V1.38.0" not in preview:
...
for marker in ["RELEASE_CONTRACT PASS", "componentNames.length===72", "actions/upload-artifact@v7", "actions/attest@v4", "artifact-metadata: write", "gh release create"]:
...
if "V1.38.0" not in app or "V1.38.0" not in login_page or "badge:72" not in components_page or "<td>Watermark</td>" not in components_page:
...
for component in ["UiAlert", "UiAnchor", "UiAutoComplete", "UiButton", "UiCalendar", "UiImage", "UiCommandPalette", "UiColorPicker", "UiIcon", "UiInput", "UiNumberInput", "UiSlider", "UiRate", "UiStatistic", "UiSelect", "UiSchemaForm", "UiTour", "UiWatermark", "UiUpload", "UiSteps", "UiTable", "UiTree", "UiSegmented", "UiDescriptions", "UiToastHost", "UiConfigProvider", "UiDateRangePicker", "toast.success"]:
...
if len(visual_baselines) < 15 or any(path.stat().st_size < 1000 for path in visual_baselines):
...
for marker in ["pixelmatch", "maxDiffRatio", "light-ltr-default", "dark-rtl-compact", "light-ltr-mobile", "managed-form-error", "schema-form", "schema-form-list", "upload-queue", "scoped-theme", "scoped-theme-portal", "scoped-motion", "api-reference", "anchor-navigation", "watermark-document", "LAN_UI_BROWSER_PATH"]:
...
if dist_lib.exists():
    ...
    if len(component_js) != 72 or len(component_types) != 72:
    ...
    if len(style_files) != 72 or not (dist_lib / "styles/core.css").is_file() or not (dist_lib / "styles/manifest.json").is_file():
