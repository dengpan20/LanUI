...
if any(marker not in preview for marker in ["V1.56.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.56.0" or package.get("private") is not False or package.get("license") != "MIT":
...
for marker in ["--ignore-workspace", "--offline", "packedManifest.private===false", "renderToString", "typescript/bin/tsc", "internals=absent", "componentNames.length===89", "PACKED_CAROUSEL_SSR PASS", "PACKED_TIME_RANGE_SSR PASS", "PACKED_DATE_TIME_SSR PASS", "PACKED_QR_CODE_SSR PASS", "PACKED_BARCODE_SSR PASS", "PACKED_KEY_VALUE_EDITOR_SSR PASS", "PACKED_PAGE_HEADER_SSR PASS", "PACKED_CARD_SSR PASS", "PACKED_TAG_SSR PASS", "allowedTopLevel", "reproducible=pass", "allowList=pass"]:
...
if distribution_budgets != {"packedFiles": 402, "packedTarballRaw": 476000, "packedUnpackedRaw": 2710000}:
    failures.append("p39:distribution-budgets")
if "Tag P60" not in components_page or "1.56.0" not in components_page or "V1.56.0" not in preview:
...
if "V1.56.0" not in app or "V1.56.0" not in login_page or "badge:89" not in components_page or "<td>Watermark</td>" not in components_page:
...
if len(visual_baselines) < 33 or any(path.stat().st_size < 1000 for path in visual_baselines):
...
if performance_budgets.get("releaseBaseline", {}).get("enhancementAllowance", {}) != {"packageJsRaw": 6000, "packageJsGzip": 2100, "packageCssRaw": 13000, "packageCssGzip": 2800, "rootCssRaw": 6000, "rootCssGzip": 1100, "subpathConsumerCssRaw": 100, "standaloneExampleJsRaw": 6500, "standaloneExampleCssRaw": 5600}:
    failures.append("performance:p60-bounded-component-enhancement-policy")
