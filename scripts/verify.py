...
required = [
    ...
    "src/components/UiIcon.vue", "src/components/UiInput.vue", "src/components/UiNumberInput.vue", "src/components/UiSlider.vue", "src/components/UiRate.vue", "src/components/UiStatistic.vue", "src/components/UiAutoComplete.vue", "src/components/UiSelect.vue", "src/components/UiTextarea.vue",
...
if api_manifest.get("schemaVersion") != 2 or len(api_manifest.get("root", {}).get("typeExports", [])) < 412 or len(manifest_components) != 62:
...
if len(style_components) != 62 or any(not item.get("bytes") or not item.get("rules") for item in style_components):
...
for marker in ['id="statistic"', "Statistic / KPI", "ui-statistic-trend", "ui-statistic-skeleton", "v1.19"]:
    if marker not in preview:
        failures.append(f"preview:statistic:{marker}")

components_page = (ROOT / "src/pages/ComponentsPage.vue").read_text(encoding="utf-8")
for component in ["UiCheckbox", "UiRadio", "UiSwitch", "UiTooltip", "UiPopover", "UiPopconfirm", "UiMenu", "UiCollapse", "UiDescriptions", "UiResult", "UiSpin", "UiSegmented", "UiConfigProvider", "UiDateRangePicker", "UiTimePicker", "UiIcon", "UiNumberInput", "UiSlider", "UiRate", "UiStatistic", "UiAutoComplete", "UiTree", "UiCommandPalette", "UiColorPicker"]:
...
for component in ["UiAlert", "UiAutoComplete", "UiButton", "UiCommandPalette", "UiColorPicker", "UiIcon", "UiInput", "UiNumberInput", "UiSlider", "UiRate", "UiStatistic", "UiSelect", "UiSteps", "UiTable", "UiTree", "UiSegmented", "UiDescriptions", "UiToastHost", "UiConfigProvider", "UiDateRangePicker", "toast.success"]:
...
direction_sources = "\n".join((ROOT / rel).read_text(encoding="utf-8") for rel in [
    ...
    "src/components/UiTree.vue", "src/components/UiTreeSelect.vue", "src/components/UiCascader.vue", "src/components/UiDrawer.vue", "src/components/UiTable.vue", "src/components/UiCommandPalette.vue", "src/components/UiColorPicker.vue", "src/components/UiRate.vue",
])
...
for marker in ["axe.run", "wcag22aa", "best-practice", "violations", "incomplete", "autocomplete-open", "multi-select-open", "tree-select-open", "tree-enterprise", "cascader-open", "command-palette-open", "color-picker-open", "modal-open", "drawer-rtl-open"]:
    if marker not in accessibility_script:
        failures.append(f"accessibility:browser:{marker}")
if accessibility_script.count("{name:") < 14:
...
if dist_lib.exists():
    ...
    if len(component_js) != 62 or len(component_types) != 62:
    ...
    if len(style_files) != 62 or not (dist_lib / "styles/core.css").is_file() or not (dist_lib / "styles/manifest.json").is_file():
