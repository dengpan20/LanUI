...
if api_manifest.get("schemaVersion") != 2 or len(api_manifest.get("root", {}).get("typeExports", [])) < 295 or len(manifest_components) != 60:
...
if len(style_components) != 60 or any(not item.get("bytes") or not item.get("rules") for item in style_components):
...
if preview.count('class="preview-section"') < 18:
...
for component in ["UiCheckbox", "UiRadio", "UiSwitch", "UiTooltip", "UiPopover", "UiPopconfirm", "UiMenu", "UiCollapse", "UiDescriptions", "UiResult", "UiSpin", "UiSegmented", "UiConfigProvider", "UiDateRangePicker", "UiTimePicker", "UiIcon", "UiNumberInput", "UiSlider", "UiAutoComplete", "UiTree", "UiCommandPalette", "UiColorPicker"]:
...
for component in ["UiAlert", "UiAutoComplete", "UiButton", "UiCommandPalette", "UiColorPicker", "UiIcon", "UiInput", "UiNumberInput", "UiSlider", "UiSelect", "UiSteps", "UiTable", "UiTree", "UiSegmented", "UiDescriptions", "UiToastHost", "UiConfigProvider", "UiDateRangePicker", "toast.success"]:
...
direction_sources = "\n".join((ROOT / rel).read_text(encoding="utf-8") for rel in [
    ...
    "src/components/UiTree.vue", "src/components/UiTreeSelect.vue", "src/components/UiCascader.vue", "src/components/UiDrawer.vue", "src/components/UiTable.vue", "src/components/UiCommandPalette.vue", "src/components/UiColorPicker.vue",
])
...
if accessibility_script.count("{name:") < 13:
...
if dist_lib.exists():
    ...
    if len(component_js) != 60 or len(component_types) != 60:
    ...
    if len(style_files) != 60 or not (dist_lib / "styles/core.css").is_file() or not (dist_lib / "styles/manifest.json").is_file():
