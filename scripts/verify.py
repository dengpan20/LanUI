...
required = [
    ...
    "LICENSE", "scripts/packed-consumer-regression.mjs", "scripts/build_p39_artifacts.py", "scripts/build_p41_artifacts.py", "scripts/build_p42_artifacts.py", "scripts/build_p43_artifacts.py", "scripts/build_p44_artifacts.py",
    ...
    "src/components/UiIcon.vue", "src/components/UiInput.vue", "src/components/UiNumberInput.vue", "src/components/UiSlider.vue", "src/components/UiRate.vue", "src/components/UiStatistic.vue", "src/components/UiCalendar.vue", "src/components/UiImage.vue", "src/components/UiDataGrid.vue", "src/components/UiVirtualList.vue", "src/components/UiStatusPage.vue", "src/components/UiAutoComplete.vue", "src/components/UiSelect.vue", "src/components/UiTextarea.vue",
    ...
    "src/components/UiNotification.vue", "src/components/UiTour.vue", "src/components/UiWatermark.vue", "src/components/UiAffix.vue", "src/components/UiSplitter.vue", "src/components/UiFormItem.vue", "src/components/UiFormList.vue", "src/components/UiSchemaForm.vue",
...
if api_manifest.get("schemaVersion") != 3 or len(api_manifest.get("root", {}).get("typeExports", [])) < 490 or len(manifest_components) != 74:
...
if len(api_categories) != 6 or len(api_components) != 74 or sum(item.get("count", 0) for item in api_categories) != 74 or len({item.get("name") for item in api_components}) != 74:
...
if len(style_components) != 74 or any(not item.get("bytes") or not item.get("rules") for item in style_components):
...
if any(marker not in preview for marker in ["V1.40.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.40.0" or package.get("private") is not False or package.get("license") != "MIT":
...
for marker in ["--ignore-workspace", "--offline", "packedManifest.private===false", "renderToString", "typescript/bin/tsc", "internals=absent", "componentNames.length===74", "allowedTopLevel", "reproducible=pass", "allowList=pass"]:
...
if distribution_budgets != {"packedFiles": 345, "packedTarballRaw": 345000, "packedUnpackedRaw": 1850000}:
    failures.append("p39:distribution-budgets")
if "Splitter P44" not in components_page or "1.40.0" not in components_page or "V1.40.0" not in preview:
...
for marker in ["RELEASE_CONTRACT PASS", "componentNames.length===74", "actions/upload-artifact@v7", "actions/attest@v4", "artifact-metadata: write", "gh release create"]:
...
if "V1.40.0" not in app or "V1.40.0" not in login_page or "badge:74" not in components_page or "<td>Watermark</td>" not in components_page:
...
for component in ["UiAffix", "UiAlert", "UiAnchor", "UiAutoComplete", "UiButton", "UiCalendar", "UiImage", "UiCommandPalette", "UiColorPicker", "UiIcon", "UiInput", "UiNumberInput", "UiSlider", "UiRate", "UiSplitter", "UiStatistic", "UiSelect", "UiSchemaForm", "UiTour", "UiWatermark", "UiUpload", "UiSteps", "UiTable", "UiTree", "UiSegmented", "UiDescriptions", "UiToastHost", "UiConfigProvider", "UiDateRangePicker", "toast.success"]:
...
if len(visual_baselines) < 17 or any(path.stat().st_size < 1000 for path in visual_baselines):
...
for marker in ["pixelmatch", "maxDiffRatio", "light-ltr-default", "dark-rtl-compact", "light-ltr-mobile", "managed-form-error", "schema-form", "schema-form-list", "upload-queue", "scoped-theme", "scoped-theme-portal", "scoped-motion", "api-reference", "anchor-navigation", "watermark-document", "affix-container", "splitter-workspace", "LAN_UI_BROWSER_PATH"]:
...
if performance_budgets.get("releaseBaseline", {}).get("componentCount") != 69 or performance_budgets.get("releaseBaseline", {}).get("perComponentAllowance", {}).get("packageJsGzip") != 1000:
    failures.append("performance:p44-additive-component-policy")
if "tolerance" in performance_budgets.get("releaseBaseline", {}):
    failures.append("performance:p44-additive-policy-must-not-use-percent-tolerance")
...
if dist_lib.exists():
    ...
    if len(component_js) != 74 or len(component_types) != 74:
    ...
    if len(style_files) != 74 or not (dist_lib / "styles/core.css").is_file() or not (dist_lib / "styles/manifest.json").is_file():
