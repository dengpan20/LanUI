...
if any(marker not in preview for marker in ["V1.57.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.57.0" or package.get("private") is not False or package.get("license") != "MIT":
...
if distribution_budgets != {"packedFiles": 402, "packedTarballRaw": 487000, "packedUnpackedRaw": 2780000}:
    failures.append("p39:distribution-budgets")
if "Timeline P61" not in components_page or "1.57.0" not in components_page or "V1.57.0" not in preview:
...
if "V1.57.0" not in app or "V1.57.0" not in login_page or "badge:89" not in components_page or "<td>Watermark</td>" not in components_page:
...
if len(visual_baselines) < 34 or any(path.stat().st_size < 1000 for path in visual_baselines):
...
if performance_budgets.get("releaseBaseline", {}).get("enhancementAllowance", {}) != {"packageJsRaw": 14500, "packageJsGzip": 4200, "packageCssRaw": 29000, "packageCssGzip": 5200, "rootCssRaw": 14000, "rootCssGzip": 2200, "subpathConsumerCssRaw": 100, "standaloneExampleJsRaw": 16000, "standaloneExampleCssRaw": 14500, "subpathConsumerJsRaw": 100}:
    failures.append("performance:p61-bounded-component-enhancement-policy")
