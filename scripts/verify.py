...
required = [
    ...
    "src/components/UiDatePicker.vue", "src/components/UiDateTimePicker.vue", "src/components/UiDateTimeRangePicker.vue", "src/components/UiTimePicker.vue", "src/components/UiTimeRangePicker.vue", "src/components/UiQRCode.vue", "src/components/UiBarcode.vue", "src/components/UiCronEditor.vue", "src/components/UiKeyValueEditor.vue", "src/components/cronUtils.js", "src/components/UiPagination.vue",
    ...
    "src/components/UiBreadcrumb.vue", "src/components/UiPageHeader.vue", "src/components/UiAvatar.vue", "src/components/UiBadge.vue",
...
non_select_vue_sources = "\n".join(path.read_text(encoding="utf-8") for path in vue_source_paths if path.name not in {"UiSelect.vue", "UiMultiSelect.vue", "UiTreeSelect.vue", "UiCascader.vue", "UiTransfer.vue"})
if "<select" in non_select_vue_sources or 'class="ui-select-native"' not in (ROOT / "src/components/UiSelect.vue").read_text(encoding="utf-8") or 'class="ui-multi-select-native"' not in (ROOT / "src/components/UiMultiSelect.vue").read_text(encoding="utf-8") or 'class="ui-tree-select-native"' not in (ROOT / "src/components/UiTreeSelect.vue").read_text(encoding="utf-8") or 'class="ui-cascader-native"' not in (ROOT / "src/components/UiCascader.vue").read_text(encoding="utf-8") or 'class="ui-transfer-native"' not in (ROOT / "src/components/UiTransfer.vue").read_text(encoding="utf-8"):
...
for page in ["DashboardPage.vue","WorkbenchPage.vue","DataPage.vue","AiPage.vue","GanttPage.vue","ComponentsPage.vue"]:
    if "UiPageHeader" not in (ROOT / "src/pages" / page).read_text(encoding="utf-8"):
...
behavior_markers = {
    ...
    "tooltip": ["@mouseenter=\"activate('hover',$event)\"", "@focusin=\"activate('focus',$event)\"", "activeReasons", "closeOnEscape", "open-change", 'role="tooltip"'],
    "popover": ["activeReasons", "closeOnOutside", "closeOnEscape", "closeOnContentClick", "trapFocus", "syncTrigger", "ui-popover-arrow", "defineExpose"],
    ...
    "select": ["data-ui-select", "useId()", "fieldNames", "remoteMethod", "requestSequence", "ui-select-native", "aria-activedescendant", "aria-controls", "ArrowDown", "Home", "End", "defineExpose"],
...
for marker in ["defaultOperatorDefinitions", "availableOperators", "addGroup", "duplicateNode", "onRuleKeydown", "collectErrors", "matches(record", "role=\"group\"", "aria-live=\"polite\"", "defineExpose"]:
    if marker not in query_builder_source:
        failures.append(f"p50:query-builder-runtime:{marker}")
if "UiQueryBuilder" not in components_page or "data-query-builder-state-contract" not in components_page or 'id="query-builder" class="preview-section"' not in preview or "UiQueryBuilder" not in (ROOT / "examples/standalone-vue/src/App.vue").read_text(encoding="utf-8"):
    failures.append("p50:showcase-consumers")
if "<td>QueryBuilder</td>" not in components_page or "query-builder-recursive-keyboard-evaluate" not in (ROOT / "scripts/interaction-regression.mjs").read_text(encoding="utf-8"):
...
for marker in ["aria-roledescription=\"carousel\"", "aria-roledescription=\"slide\"", "props.effect === 'fade'", "visibilitychange", "pointerdown", "useReducedMotion", "useDirection", "defineExpose"]:
    if marker not in carousel_source:
        failures.append(f"p51:carousel-runtime:{marker}")
if "UiCarousel" not in components_page or "data-carousel-state-contract" not in components_page or 'id="carousel" class="preview-section"' not in preview or "UiCarousel" not in (ROOT / "examples/standalone-vue/src/App.vue").read_text(encoding="utf-8"):
...
for marker in ["date.startTime", "date.endTime", "date.timeRangeLabel", "rangeLabel"]:
    if marker not in date_range_source:
        failures.append(f"p52:localized-range:{marker}")
for marker in ["'time-range': UiTimeRangePicker", "datetime: UiDatePicker", "'datetime-range': UiDateRangePicker", "builtinDefaultProps"]:
    if marker not in schema_form_source:
        failures.append(f"p52:schema-form:{marker}")
if "UiTimeRangePicker" not in components_page or "data-time-range-state-contract" not in preview or "UiTimeRangePicker" not in (ROOT / "examples/standalone-vue/src/App.vue").read_text(encoding="utf-8"):
...
date_time_range_source = (ROOT / "src/components/UiDateTimeRangePicker.vue").read_text(encoding="utf-8")
...
if any(marker not in components_page for marker in ["UiDateTimePicker", "UiDateTimeRangePicker", "日期时间", "发布窗口"]):
    failures.append("p53:component-center")
if "data-date-time-state-contract" not in preview or any(marker not in standalone_source for marker in ["UiDateTimePicker", "UiDateTimeRangePicker"]):
...
for marker in ["qrcode-generator", "getModuleCount", "shape-rendering=\"crispEdges\"", "statuses=new Set", "URL.createObjectURL", "defineExpose", "aria-labelledby"]:
    if marker not in qr_code_source:
        failures.append(f"p54:qr-code-runtime:{marker}")
if package.get("dependencies", {}).get("qrcode-generator") != "2.0.4":
    failures.append("p54:encoder-dependency")
if any(marker not in components_page for marker in ["UiQRCode", "data-qr-code-state-contract", "<td>QRCode</td>"]):
    failures.append("p54:component-center")
if any(marker not in preview for marker in ['id="qr-code"', "previewQrRows", "previewQrDownload", "data-qr-code-state-contract"]):
    failures.append("p54:static-preview")
if "UiQRCode" not in standalone_source or "standaloneQrStatus" not in standalone_source:
...
for marker in ["jsbarcode", "shape-rendering=\"crispEdges\"", "formats=new Map", "statuses=new Set", "URL.createObjectURL", "getEncoding", "defineExpose", "aria-labelledby"]:
    if marker not in barcode_source:
...
if "UiBarcode" not in standalone_source or "standaloneBarcodeStatus" not in standalone_source:
...
for marker in ["validationFor", "importText", "allowDuplicateKeys", "keyPattern", "data-valid", "aria-invalid", "inputName", "defineExpose"]:
    if marker not in key_value_source:
        failures.append(f"p57:key-value-runtime:{marker}")
if any(marker not in components_page for marker in ["UiKeyValueEditor", "KeyValueEditor P57", "data-key-value-editor-state-contract", "<td>KeyValueEditor</td>"]):
    failures.append("p57:component-center")
if any(marker not in preview for marker in ['id="key-value-editor"', "previewKeyValueRows", "renderPreviewKeyValue", "data-key-value-editor-state-contract"]):
    failures.append("p57:static-preview")
if "UiKeyValueEditor" not in standalone_source or "standaloneHeaders" not in standalone_source:
...
for marker in ["data-ui-page-header", "aria-labelledby", "breadcrumbLabel", "breadcrumb-navigate", "showBack", "stickyOffset", "useReducedMotion", "focusBack", "scrollIntoView", "defineExpose"]:
    if marker not in page_header_source:
        failures.append(f"p58:page-header-runtime:{marker}")
if any(marker not in components_page for marker in ["UiPageHeader", "PageHeader P58", "data-page-header-state-contract"]):
    failures.append("p58:component-center")
if any(marker not in preview for marker in ['id="page-header"', "previewPageHeaderBack", "previewPageHeaderOutput", "data-page-header-state-contract"]):
    failures.append("p58:static-preview")
if "UiPageHeader" not in standalone_source or "standalone-tour-trigger" not in standalone_source:
...
for page_name in ["DashboardPage.vue", "WorkbenchPage.vue", "DataPage.vue", "AiPage.vue", "GanttPage.vue", "ComponentsPage.vue"]:
    ...
    if "UiPageHeader" not in page_source or "page-heading" in page_source:
        failures.append(f"p58:page-migration:{page_name}")

card_source = (ROOT / "src/components/UiCard.vue").read_text(encoding="utf-8")
for marker in ["data-ui-card", "aria-labelledby", "aria-describedby", "aria-busy", "aria-pressed", "loadingRows", "isNestedControl", "useReducedMotion", "scrollIntoView", "defineExpose"]:
    if marker not in card_source:
        failures.append(f"p59:card-runtime:{marker}")
if any(marker not in components_page for marker in ["Card P59", "data-card-state-contract", "cardSelected"]):
    failures.append("p59:component-center")
if any(marker not in preview for marker in ['id="card-component"', "previewCardOutput", "data-card-state-contract"]):
    failures.append("p59:static-preview")
if any(marker not in standalone_source for marker in ["UiCard", "standaloneCardSelected", "data-card-state-contract"]):
...
for marker in ["data-ui-tag", "variant", "closable", "checkable", "aria-pressed", "update:checked", "tag.close", "noopener noreferrer", "focusClose", "defineExpose"]:
    if marker not in tag_source:
        failures.append(f"p60:tag-runtime:{marker}")
if any(marker not in components_page for marker in ["Tag P60", "data-tag-state-contract", "tagChecked", "tagVisible"]):
    failures.append("p60:component-center")
if any(marker not in preview for marker in ['id="tag-component"', "previewTagToggle", "previewTagClose", "data-tag-state-contract"]):
    failures.append("p60:static-preview")
if any(marker not in standalone_source for marker in ["UiTag", "standaloneTagChecked", "standaloneTagVisible", "data-tag-state-contract"]):
...
for marker in ["data-ui-timeline", "timePosition", "disabledKeys", "useDirection", "aria-busy", "timeline.pending", "focusItem", "defineExpose"]:
    if marker not in timeline_source:
        failures.append(f"p61:timeline-runtime:{marker}")
if any(marker not in components_page for marker in ["Timeline P61", "data-timeline-state-contract", "timelineSelection", "time-position=\"opposite\""]):
    failures.append("p61:component-center")
if any(marker not in preview for marker in ['id="timeline-component"', "previewTimelineActions", "data-timeline-state-contract"]):
    failures.append("p61:static-preview")
if any(marker not in standalone_source for marker in ["UiTimeline", "standaloneTimelineSelection", "data-timeline-state-contract"]):
...
for marker in ["data-ui-steps", "ui-step-connector", "itemKey", "modelValue", "defaultCurrent", "labelPlacement", "type", "linear", "disabledIndexes", "useDirection", "aria-busy", "steps.empty", "focusCurrent", "defineExpose"]:
    if marker not in steps_source:
        failures.append(f"p62:steps-runtime:{marker}")
if any(marker not in components_page for marker in ["Steps P62", "data-steps-state-contract", "stepsCurrent", "type=\"navigation\""]):
    failures.append("p62:component-center")
if any(marker not in preview for marker in ['id="steps-component"', "previewSteps", "data-steps-state-contract", "ui-step-connector"]):
    failures.append("p62:static-preview")
if any(marker not in standalone_source for marker in ["UiSteps", "deliveryStep", "type=\"navigation\"", "linear"]):
...
for marker in ["data-ui-breadcrumb", "ui-breadcrumb-overflow", "itemKey", "currentKey", "maxItems", "itemsBeforeCollapse", "itemsAfterCollapse", "expanded", "defaultExpanded", "separatorMode", "loading", "emptyText", "update:expanded", "expand-change", "focusItem", "navigateItem", "defineExpose"]:
    if marker not in breadcrumb_source:
        failures.append(f"p63:breadcrumb-runtime:{marker}")
if any(marker not in components_page for marker in ["Breadcrumb P63", "data-breadcrumb-state-contract", "breadcrumbExpanded", ":max-items"]):
    failures.append("p63:component-center")
if any(marker not in preview for marker in ['id="breadcrumb-component"', "previewBreadcrumbOverflow", "data-breadcrumb-state-contract", "ui-breadcrumb-overflow"]):
    failures.append("p63:static-preview")
if any(marker not in standalone_source for marker in ["UiBreadcrumb", "standaloneBreadcrumbExpanded", "data-breadcrumb-state-contract"]):
...
for marker in ["activeReasons", "defaultOpen", "showDelay", "hideDelay", "closeOnEscape", "closeOnOutside", "open-change", "ui-tooltip-arrow", "syncDescription", "defineExpose"]:
    if marker not in tooltip_source:
        failures.append(f"p64:tooltip-runtime:{marker}")
if any(marker not in components_page for marker in ["Tooltip P64", "data-tooltip-showcase", "tooltipDemoOpen", "show-delay"]):
    failures.append("p64:component-center")
if any(marker not in preview for marker in ['id="tooltip-component"', "previewClickTooltipTrigger", "data-tooltip-state-contract", "ui-tooltip-arrow"]):
    failures.append("p64:static-preview")
if any(marker not in standalone_source for marker in ["UiTooltip", "standaloneTooltipOpen", "data-tooltip-state-contract"]):
...
for marker in ["activeReasons", "defaultOpen", "showDelay", "hideDelay", "closeOnEscape", "closeOnOutside", "closeOnContentClick", "trapFocus", "syncTrigger", "open-change", "ui-popover-arrow", "defineExpose"]:
    if marker not in popover_source:
        failures.append(f"p65:popover-runtime:{marker}")
if any(marker not in components_page for marker in ["Popover P65", "data-popover-showcase", "popoverDemoOpen", "trap-focus"]):
    failures.append("p65:component-center")
if any(marker not in preview for marker in ['id="popover-component"', "previewMaturePopoverTrigger", "data-popover-state-contract", "ui-popover-arrow"]):
    failures.append("p65:static-preview")
if any(marker not in standalone_source for marker in ["UiPopover", "standalonePopoverOpen", "data-popover-state-contract"]):
...
for marker in ["activeReasons", "defaultOpen", "defaultActiveIndex", "showDelay", "hideDelay", "closeOnEscape", "closeOnOutside", "closeOnSelect", "typeaheadBuffer", "focusAdjacentTrigger", "menuitemcheckbox", "open-change", "active-change", "defineExpose"]:
    if marker not in dropdown_source:
        failures.append(f"p66:dropdown-runtime:{marker}")
if any(marker not in components_page for marker in ["Dropdown P66", "data-dropdown-showcase", "dropdownDemoOpen", "Typeahead"]):
    failures.append("p66:component-center")
if any(marker not in preview for marker in ['id="dropdown-component"', "previewMatureDropdownTrigger", "data-dropdown-state-contract", "previewDropdownTypeahead"]):
    failures.append("p66:static-preview")
if any(marker not in standalone_source for marker in ["UiDropdown", "standaloneDropdownOpen", "data-dropdown-state-contract"]):
...
collapse_source = (ROOT / "src/components/UiCollapse.vue").read_text(encoding="utf-8")
for marker in ["defaultValue", "collapsible", "enforceOpenKeys", "beforeToggle", "destroyOnHide", "mountedKeys", "ArrowDown", "pendingKeys", "focusFirst", "after-open", "defineExpose"]:
    if marker not in collapse_source:
        failures.append(f"p67:collapse-runtime:{marker}")
if any(marker not in components_page for marker in ["Collapse P67", "data-collapse-state-contract", "collapseDemo", "validateCollapseToggle"]):
    failures.append("p67:component-center")
if any(marker not in preview for marker in ['id="collapse-component"', "previewMatureCollapse", "data-collapse-state-contract", "togglePreviewMatureCollapse"]):
    failures.append("p67:static-preview")
if any(marker not in standalone_source for marker in ["UiCollapse", "standaloneCollapseOpen", "data-collapse-state-contract"]):
...
for marker in ["modelModifiers", "formatter", "parser", "clearOnEscape", "passwordVisible", "composition-end", "data-ui-input", "defineExpose"]:
    if marker not in input_source:
        failures.append(f"p69:input-runtime:{marker}")
if any(marker not in components_page for marker in ["Input P69", "data-input-state-contract", "passwordVisible", "tenantDomain"]):
    failures.append("p69:component-center")
if any(marker not in preview for marker in ["data-input-state-contract", "previewMatureInput", "previewMaturePassword", "syncPreviewMatureInput"]):
    failures.append("p69:static-preview")
if any(marker not in standalone_source for marker in ["UiInput", "standaloneInputState", "standalonePasswordVisible", "data-input-state-contract"]):
...
for marker in ["modelModifiers", "formatter", "parser", "autoSize", "clearOnEscape", "submitOnEnter", "composition-end", "data-ui-textarea", "defineExpose"]:
    if marker not in textarea_source:
        failures.append(f"p70:textarea-runtime:{marker}")
if any(marker not in components_page for marker in ["Textarea P70", "data-textarea-state-contract", "textareaTemplate", "textareaContractState"]):
    failures.append("p70:component-center")
if any(marker not in preview for marker in ["data-textarea-state-contract", "previewMatureTextarea", "syncPreviewTextarea"]):
    failures.append("p70:static-preview")
if any(marker not in standalone_source for marker in ["UiTextarea", "standaloneTextareaState", "data-textarea-state-contract"]):
...
for marker in ["defaultValue", "defaultOpen", "fieldNames", "filterOption", "remoteMethod", "remoteDebounce", "AbortController", "requestSequence", "remoteCache", "ui-select-native", "onFormReset", "handleTypeahead", "useTeleportThemeScope", "defineExpose"]:
    if marker not in select_source:
        failures.append(f"p72:select-runtime:{marker}")
if any(marker not in components_page for marker in ["UiSelect · P72", "data-select-state-contract", "selectFieldOptions", "fetchRegionOptions", "selectContractState"]):
    failures.append("p72:component-center")
if any(marker not in preview for marker in ["UiSelect P72", "data-select-state-contract", "previewSelectNative", "previewSelectSearch", "syncPreviewSelectActive"]):
    failures.append("p72:static-preview")
if any(marker not in standalone_source for marker in ["UiSelect", "standaloneSelectOptions", "fetchStandaloneSelect", "data-select-state-contract"]):
...
for marker in ["defaultValue", "defaultOpen", "fieldNames", "filterOption", "remoteMethod", "remoteDebounce", "AbortController", "requestSequence", "remoteCache", "maxCount", "minCount", "showSelectAll", "ui-multi-select-native", "onFormReset", "handleTypeahead", "useTeleportThemeScope", "defineExpose"]:
    if marker not in multi_select_source:
        failures.append(f"p73:multi-select-runtime:{marker}")
if any(marker not in components_page for marker in ["MultiSelect P73", "data-multi-select-state-contract", "advancedOptions", "fetchMultiDemo", "multiState"]):
    failures.append("p73:component-center")
if any(marker not in preview for marker in ["UiMultiSelect P73", "data-multi-select-state-contract", "ui-multi-select-trigger", "ui-multi-overflow-tag"]):
    failures.append("p73:static-preview")
if any(marker not in standalone_source for marker in ["UiMultiSelect", "standaloneMultiSelect", "data-multi-select-state-contract"]):
...
for marker in ["defaultValue", "defaultOpen", "expandedKeys", "defaultExpandedKeys", "fieldNames", "filterNode", "loadData", "AbortController", "loadSequences", "maxCount", "minCount", "checkStrictly", "ui-tree-select-native", "onFormReset", "handleTypeahead", "useTeleportThemeScope", "defineExpose"]:
    if marker not in tree_select_source:
        failures.append(f"p74:tree-select-runtime:{marker}")
if any(marker not in components_page for marker in ["TreeSelect P74", "data-tree-select-state-contract", "treeOptions", "loadTreeDemo", "treeState"]):
    failures.append("p74:component-center")
if any(marker not in preview for marker in ["UiTreeSelect P74", "data-tree-select-state-contract", "previewTreeSelectNative", "previewTreeSelectSearch", "syncPreviewTreeSelectActive"]):
    failures.append("p74:static-preview")
if any(marker not in standalone_source for marker in ["UiTreeSelect", "standaloneTreeSelect", "loadStandaloneTree", "data-tree-select-state-contract"]):
    failures.append("p74:standalone-consumer")
if "tree-select-production-hierarchy-search-lazy-native-api" not in (ROOT / "scripts/interaction-regression.mjs").read_text(encoding="utf-8") or "tree-select-contract" not in (ROOT / "scripts/visual-regression.mjs").read_text(encoding="utf-8") or "tree-select-contract" not in (ROOT / "scripts/accessibility-regression.mjs").read_text(encoding="utf-8"):
    failures.append("p74:state-and-interaction")
if any(marker not in (ROOT / "tests/types/contracts.ts").read_text(encoding="utf-8") for marker in ["UiTreeSelectProps", "UiTreeSelectInstance", "invalidTreeSelectLoader"]):
...
for marker in ["defaultValue", "selectedKeys", "defaultSelectedKeys", "searchValues", "defaultSearchValues", "fieldNames", "filterOption", "showSelectAll", "oneWay", "targetOrder", "minCount", "maxCount", "UiVirtualList", "ui-transfer-native", "onFormReset", "compositionend", "defineExpose"]:
    if marker not in transfer_source:
        failures.append(f"p76:transfer-runtime:{marker}")
if any(marker not in components_page for marker in ["UiTransfer · P76", "data-transfer-state-contract", "transferOptions", "transferSelected", "transferSearch"]):
    failures.append("p76:component-center")
if any(marker not in preview for marker in ["UiTransfer P76", "data-transfer-state-contract", "previewTransferNative", "previewTransferSearch", "renderPreviewTransfer"]):
    failures.append("p76:static-preview")
if any(marker not in standalone_source for marker in ["UiTransfer", "standaloneTransferOptions", "standaloneTransferSelected", "data-transfer-state-contract"]):
    failures.append("p76:standalone-consumer")
if "transfer-production-search-limits-virtual-native-api" not in (ROOT / "scripts/interaction-regression.mjs").read_text(encoding="utf-8") or "transfer-contract" not in (ROOT / "scripts/visual-regression.mjs").read_text(encoding="utf-8") or "transfer-contract" not in (ROOT / "scripts/accessibility-regression.mjs").read_text(encoding="utf-8"):
    failures.append("p76:state-and-interaction")
if any(marker not in (ROOT / "tests/types/contracts.ts").read_text(encoding="utf-8") for marker in ["UiTransferProps", "UiTransferInstance", "invalidTransferOrder"]):
    failures.append("p76:type-contract")
if any(marker not in packed_consumer for marker in ["PACKED_TRANSFER_SSR PASS", "transfer-contracts.ts"]):
    failures.append("p76:packed-consumer")
if "Maturity P76: production transfer controls" not in (ROOT / "UI-SPEC.md").read_text(encoding="utf-8"):
    failures.append("p76:documentation")

pagination_source = (ROOT / "src/components/UiPagination.vue").read_text(encoding="utf-8")
for marker in ["defaultPage", "defaultPageSize", "pagerCount", "showQuickJumper", "showFirstLast", "ellipsisInteractive", "pageSizeChangeBehavior", "beforeChange", "ResizeObserver", "onRootKeydown", "defineExpose"]:
    if marker not in pagination_source:
        failures.append(f"p77:pagination-runtime:{marker}")
if any(marker not in components_page for marker in ["UiPagination · 生产级分页", "data-pagination-state-contract", "paginationState", "show-quick-jumper", "preserve-item"]):
    failures.append("p77:component-center")
if any(marker not in preview for marker in ["UiPagination P77", "data-ui-pagination", "previewPaginationSize", "previewPaginationJump", "updatePreviewPagination"]):
    failures.append("p77:static-preview")
if any(marker not in standalone_source for marker in ["UiPagination", "standalonePaginationPage", "standalonePaginationSize", "Standalone release pages"]):
    failures.append("p77:standalone-consumer")
if "pagination-production-window-quick-size-guard-api-rtl" not in (ROOT / "scripts/interaction-regression.mjs").read_text(encoding="utf-8") or "pagination-contract" not in (ROOT / "scripts/visual-regression.mjs").read_text(encoding="utf-8") or "pagination-contract" not in (ROOT / "scripts/accessibility-regression.mjs").read_text(encoding="utf-8"):
    failures.append("p77:state-and-interaction")
if any(marker not in (ROOT / "tests/types/contracts.ts").read_text(encoding="utf-8") for marker in ["UiPaginationProps", "UiPaginationInstance", "invalidPaginationBehavior"]):
    failures.append("p77:type-contract")
if any(marker not in packed_consumer for marker in ["PACKED_PAGINATION_SSR PASS", "pagination-contracts.ts", "styles/UiPagination.css"]):
    failures.append("p77:packed-consumer")
if "Maturity P77: production pagination controls" not in (ROOT / "UI-SPEC.md").read_text(encoding="utf-8"):
    failures.append("p77:documentation")

table_source = (ROOT / "src/components/UiTable.vue").read_text(encoding="utf-8")
for marker in ["defaultSelectedRows", "beforeSelect", "defaultExpandedRows", "beforeExpand", "defaultCurrentRowKey", "defaultColumnWidths", "defaultFilterOpenKey", "selectOnRowClick", "expandOnRowClick", "selectionMode", "onFilterMenuKeydown", "beginResize", "resizeByKeyboard", "virtualEnabled", "getState", "defineExpose"]:
    if marker not in table_source:
        failures.append(f"p78:table-runtime:{marker}")
if any(marker not in components_page for marker in ["UiTable · P78 生产级数据表", "data-table-state-contract", "tableCurrent", "tableWidths", "select-on-row-click", "highlight-current-row", "@column-resize"]):
    failures.append("p78:component-center")
if any(marker not in preview for marker in ["UiTable P78", "data-table-state-contract", "previewTableState", "data-row-key", "activatePreviewTableRow"]):
    failures.append("p78:static-preview")
if any(marker not in standalone_source for marker in ["standaloneTableSelected", "standaloneTableExpanded", "standaloneTableCurrent", "standaloneTableWidths", "Standalone assembly results"]):
    failures.append("p78:standalone-consumer")
if "table-production-selection-expansion-filter-resize-keyboard-api-rtl" not in (ROOT / "scripts/interaction-regression.mjs").read_text(encoding="utf-8") or "table-contract" not in (ROOT / "scripts/visual-regression.mjs").read_text(encoding="utf-8") or "table-contract" not in (ROOT / "scripts/accessibility-regression.mjs").read_text(encoding="utf-8"):
    failures.append("p78:state-and-interaction")
if any(marker not in (ROOT / "tests/types/contracts.ts").read_text(encoding="utf-8") for marker in ["UiTableProps", "UiTableInstance", "invalidTableSelection", "invalidTableSortOrders", "invalidTableLayout", "invalidTableRowKey"]):
    failures.append("p78:type-contract")
if any(marker not in packed_consumer for marker in ["PACKED_TABLE_SSR PASS", "table-contracts.ts", "styles/UiTable.css"]):
    failures.append("p78:packed-consumer")
if "Maturity P78: production table controls" not in (ROOT / "UI-SPEC.md").read_text(encoding="utf-8"):
    failures.append("p78:documentation")

date_picker_source = (ROOT / "src/components/UiDatePicker.vue").read_text(encoding="utf-8")
for marker in ["defaultValue", "defaultOpen", "defaultViewDate", "beforeChange", "beforeOpenChange", "disabledDate", "presets", "UiCalendar", "UiPopover", "focusCalendar", "defineExpose"]:
    if marker not in date_picker_source:
        failures.append(f"p79:date-picker-runtime:{marker}")
if any(marker not in components_page for marker in ["UiDatePicker · P79 生产级日期选择器", "data-date-picker-state-contract", "demoDateOpen", "demoDateView", "datePickerGuard", "show-week-numbers"]):
    failures.append("p79:component-center")
if any(marker not in preview for marker in ["UiDatePicker P79", "data-date-picker-state-contract", "previewDatePanel", "previewDateState", "selectPreviewDate"]):
    failures.append("p79:static-preview")
if any(marker not in standalone_source for marker in ["UiDatePicker", "releaseDateOpen", "releaseDateView", "releaseDatePresets", "Production DatePicker · P79"]):
    failures.append("p79:standalone-consumer")
if "date-picker-production-calendar-presets-guards-keyboard-api-rtl" not in (ROOT / "scripts/interaction-regression.mjs").read_text(encoding="utf-8") or "date-picker-contract" not in (ROOT / "scripts/visual-regression.mjs").read_text(encoding="utf-8") or "date-picker-contract" not in (ROOT / "scripts/accessibility-regression.mjs").read_text(encoding="utf-8"):
    failures.append("p79:state-and-interaction")
if any(marker not in (ROOT / "tests/types/contracts.ts").read_text(encoding="utf-8") for marker in ["UiDatePickerProps", "UiDatePickerInstance", "invalidDatePickerMode", "invalidDatePickerValueType", "invalidDatePickerPlacement", "invalidDatePickerFirstDay", "invalidDatePickerPreset"]):
    failures.append("p79:type-contract")
if any(marker not in packed_consumer for marker in ["PACKED_DATE_PICKER_SSR PASS", "date-picker-contracts.ts", "styles/UiDatePicker.css"]):
    failures.append("p79:packed-consumer")
...
direction_sources = "\n".join((ROOT / rel).read_text(encoding="utf-8") for rel in [
    "src/config.js", "src/components/UiTabs.vue", "src/components/UiSegmented.vue", "src/components/UiMenu.vue",
        "src/components/UiTree.vue", "src/components/UiTreeSelect.vue", "src/components/UiCascader.vue", "src/components/UiPagination.vue", "src/components/UiDrawer.vue", "src/components/UiTable.vue", "src/components/UiCommandPalette.vue", "src/components/UiColorPicker.vue", "src/components/UiRate.vue", "src/components/UiCalendar.vue", "src/components/UiImage.vue",
])
...
if len(visual_baselines) < 51 or any(path.stat().st_size < 1000 for path in visual_baselines):
...
print("- maturity-p50=ui-query-builder,recursive-and-or-not,typed-editors,immutable-actions,keyboard+validation,local-evaluation,form+schema,ssr+types,23-visual,41-axe,45-interactions-per-browser,35-negative-types")
print("- maturity-p51=ui-carousel,slide+fade,horizontal+vertical,loop+finite,keyboard+rtl,swipe+lazy,autoplay-pause-reasons,reduced-motion,ssr+types,24-visual,42-axe,46-interactions-per-browser,36-negative-types")
print("- maturity-p52=ui-time-range-picker,string+date+timestamp,timezone+precision,constraints+order-validation,form+schema,ssr+types,25-visual,43-axe,47-interactions-per-browser,37-negative-types")
...
print("- maturity-p57=ui-key-value-editor,controlled-records,custom-fields,add+remove+move+toggle,dotenv-import,structured-validation,form+responsive+rtl+ssr+types,30-visual,48-axe,52-interactions-per-browser,47-negative-types")
print("- maturity-p58=ui-page-header,semantic-heading,breadcrumb+back+meta+actions+footer,loading+sticky+responsive+rtl+reduced-motion+ssr+types,31-visual,49-axe,53-interactions-per-browser,49-negative-types")
print("- maturity-p59=ui-card,semantic-regions,variants+sizes+shadow,interactive+link+selected+disabled+loading,nested-action-isolation+rtl+ssr+types,32-visual,50-axe,54-interactions-per-browser,51-negative-types")
print("- maturity-p60=ui-tag,semantic+custom-colors,soft+solid+outlined,sizes+dot+round,checkable+closable+link+disabled,keyboard+rtl+ssr+types,33-visual,51-axe,55-interactions-per-browser,53-negative-types")
print("- maturity-p61=ui-timeline,field-adapters,status+custom-color+icons,vertical+horizontal,start+end+alternate,semantic-time,selection+link+disabled+pending+loading+empty,keyboard+rtl+ssr+types,34-visual,52-axe,56-interactions-per-browser,55-negative-types")
print("- maturity-p62=ui-steps,dedicated-connectors,field-adapters,controlled+uncontrolled,default+navigation+inline,horizontal+vertical+labels,linear+disabled+loading+empty,keyboard+rtl+responsive+ssr+types,35-visual,53-axe,57-interactions-per-browser,57-negative-types")
print("- maturity-p63=ui-breadcrumb,semantic-list+current-page,field-adapters,controlled+uncontrolled-collapse,links+buttons+disabled+loading+empty,slots+imperative-focus,keyboard+rtl+responsive+ssr+types,36-visual,54-axe,58-interactions-per-browser,59-negative-types")
print("- maturity-p64=ui-tooltip,hover+focus+click+manual,controlled+uncontrolled,delays+reentry,outside+escape,arrow+wrap+teleport,aria-description+rtl+ssr+types,37-visual,55-axe,59-interactions-per-browser,61-negative-types")
...
print("- maturity-p76=ui-transfer,controlled+uncontrolled,value+selection+two-sided-search,field-mapping+ime,visible-select-all,min+max+ordering+one-way,virtual-panels,native-form+reset,keyboard+rtl+ssr+types,49-visual,66-axe,71-interactions-per-browser,97-negative-types")
print("- maturity-p77=ui-pagination,controlled+uncontrolled,page+page-size,normalization+window+ellipsis,quick+simple+responsive,async-guard,size-policies,keyboard+rtl+ssr+types,50-visual,67-axe,72-interactions-per-browser,101-negative-types")
print("- maturity-p78=ui-table,controlled+uncontrolled,selection+expansion+sorting+filters+current-row,column-resize+virtualization,row-policies+async-guards,keyboard+rtl+ssr+types,51-visual,68-axe,73-interactions-per-browser,106-negative-types")
print("- maturity-p79=ui-date-picker,controlled+uncontrolled,value+open+view,calendar+popover,constraints+presets,async-guards,keyboard+rtl+portal+ssr+types,52-visual,69-axe,74-interactions-per-browser,111-negative-types")
