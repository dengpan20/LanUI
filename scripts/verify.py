...
required = [
    ...
    "src/components/UiDatePicker.vue", "src/components/UiDateTimePicker.vue", "src/components/UiDateTimeRangePicker.vue", "src/components/UiTimePicker.vue", "src/components/UiTimeRangePicker.vue", "src/components/UiPagination.vue",
...
if api_manifest.get("schemaVersion") != 3 or len(api_manifest.get("root", {}).get("typeExports", [])) < 691 or len(manifest_components) != 84:
...
if len(api_categories) != 6 or len(api_components) != 84 or sum(item.get("count", 0) for item in api_categories) != 84 or len({item.get("name") for item in api_components}) != 84:
...
if len(style_components) != 84 or any(not item.get("bytes") or not item.get("rules") for item in style_components):
...
if any(marker not in preview for marker in ["V1.49.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.49.0" or package.get("private") is not False or package.get("license") != "MIT":
...
for marker in ["--ignore-workspace", "--offline", "packedManifest.private===false", "renderToString", "typescript/bin/tsc", "internals=absent", "componentNames.length===84", "PACKED_CAROUSEL_SSR PASS", "PACKED_TIME_RANGE_SSR PASS", "PACKED_DATE_TIME_SSR PASS", "allowedTopLevel", "reproducible=pass", "allowList=pass"]:
...
if distribution_budgets != {"packedFiles": 382, "packedTarballRaw": 416000, "packedUnpackedRaw": 2350000}:
    failures.append("p39:distribution-budgets")
if "DateTime Adapters P53" not in components_page or "1.49.0" not in components_page or "V1.49.0" not in preview:
...
for marker in ["RELEASE_CONTRACT PASS", "componentNames.length===84", "actions/upload-artifact@v7", "actions/attest@v4", "artifact-metadata: write", "gh release create"]:
...
if "V1.49.0" not in app or "V1.49.0" not in login_page or "badge:84" not in components_page or "<td>Watermark</td>" not in components_page:
...
if any(marker not in components_page for marker in ["UiDateTimePicker", "UiDateTimeRangePicker", "DateTime Adapters P53"]):
...
if performance_budgets.get("releaseBaseline", {}).get("componentCount") != 69 or performance_budgets.get("releaseBaseline", {}).get("perComponentAllowance", {}) != {"packageJsRaw": 5000, "packageJsGzip": 3375, "packageCssRaw": 5750, "packageCssGzip": 850, "largestChunkRaw": 150, "largestChunkGzip": 75, "largestComponentCssRaw": 1500, "largestComponentCssGzip": 150, "standaloneExampleJsRaw": 5400}:
...
if dist_lib.exists():
    ...
    if len(component_js) != 84 or len(component_types) != 84:
    ...
    if len(style_files) != 84 or not (dist_lib / "styles/core.css").is_file() or not (dist_lib / "styles/manifest.json").is_file():
