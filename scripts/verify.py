...
required = [
    ...
    "src/components/UiIcon.vue", "src/components/UiInput.vue", "src/components/UiNumberInput.vue", "src/components/UiSlider.vue", "src/components/UiRate.vue", "src/components/UiAutoComplete.vue", "src/components/UiSelect.vue", "src/components/UiTextarea.vue",
...
if api_manifest.get("schemaVersion") != 2 or len(api_manifest.get("root", {}).get("typeExports", [])) < 299 or len(manifest_components) != 61:
...
if len(style_components) != 61 or any(not item.get("bytes") or not item.get("rules") for item in style_components):
...
if preview.count('class="preview-section"') < 19:
...
for component in ["UiCheckbox", "UiRadio", "UiSwitch", "UiTooltip", "UiPopover", "UiPopconfirm", "UiMenu", "UiCollapse", "UiDescriptions", "UiResult", "UiSpin", "UiSegmented", "UiConfigProvider", "UiDateRangePicker", "UiTimePicker", "UiIcon", "UiNumberInput", "UiSlider", "UiRate", "UiAutoComplete", "UiTree", "UiCommandPalette", "UiColorPicker"]:
...
for component in ["UiAlert", "UiAutoComplete", "UiButton", "UiCommandPalette", "UiColorPicker", "UiIcon", "UiInput", "UiNumberInput", "UiSlider", "UiRate", "UiSelect", "UiSteps", "UiTable", "UiTree", "UiSegmented", "UiDescriptions", "UiToastHost", "UiConfigProvider", "UiDateRangePicker", "toast.success"]:
...
if dist_lib.exists():
    ...
    if len(component_js) != 61 or len(component_types) != 61:
    ...
    if len(style_files) != 61 or not (dist_lib / "styles/core.css").is_file() or not (dist_lib / "styles/manifest.json").is_file():
