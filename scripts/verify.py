...
required = [
    ...
    "tests/mentions-p48.spec.js", "tests/visual/baselines/win32/mentions-contract.png",
...
if api_manifest.get("schemaVersion") != 3 or len(api_manifest.get("root", {}).get("typeExports", [])) < 646 or len(manifest_components) != 78:
...
if len(api_categories) != 6 or len(api_components) != 78 or sum(item.get("count", 0) for item in api_categories) != 78 or len({item.get("name") for item in api_components}) != 78:
...
if len(style_components) != 78 or any(not item.get("bytes") or not item.get("rules") for item in style_components):
...
if any(marker not in preview for marker in ["V1.44.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.44.0" or package.get("private") is not False or package.get("license") != "MIT":
...
for marker in ["--ignore-workspace", "--offline", "packedManifest.private===false", "renderToString", "typescript/bin/tsc", "internals=absent", "componentNames.length===78", "allowedTopLevel", "reproducible=pass", "allowList=pass"]:
...
if distribution_budgets != {"packedFiles": 358, "packedTarballRaw": 375000, "packedUnpackedRaw": 2120000}:
    failures.append("p39:distribution-budgets")
if "Mentions P48" not in components_page or "1.44.0" not in components_page or "V1.44.0" not in preview:
...
for marker in ["RELEASE_CONTRACT PASS", "componentNames.length===78", "actions/upload-artifact@v7", "actions/attest@v4", "artifact-metadata: write", "gh release create"]:
...
if "V1.44.0" not in app or "V1.44.0" not in login_page or "badge:78" not in components_page or "<td>Watermark</td>" not in components_page:
...
if len(visual_baselines) < 21 or any(path.stat().st_size < 1000 for path in visual_baselines):
...
if performance_budgets.get("releaseBaseline", {}).get("componentCount") != 69 or performance_budgets.get("releaseBaseline", {}).get("perComponentAllowance", {}).get("packageJsGzip") != 2750:
...
if dist_lib.exists():
    ...
    if len(component_js) != 78 or len(component_types) != 78:
    ...
    if len(style_files) != 78 or not (dist_lib / "styles/core.css").is_file() or not (dist_lib / "styles/manifest.json").is_file():
