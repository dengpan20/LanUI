...
required = [
    ...
    "src/components/UiDatePicker.vue", "src/components/UiTimePicker.vue", "src/components/UiTimeRangePicker.vue", "src/components/UiPagination.vue",
...
if api_manifest.get("schemaVersion") != 3 or len(api_manifest.get("root", {}).get("typeExports", [])) < 685 or len(manifest_components) != 82:
...
if len(api_categories) != 6 or len(api_components) != 82 or sum(item.get("count", 0) for item in api_categories) != 82 or len({item.get("name") for item in api_components}) != 82:
...
if len(style_components) != 82 or any(not item.get("bytes") or not item.get("rules") for item in style_components):
...
if any(marker not in preview for marker in ["V1.48.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.48.0" or package.get("private") is not False or package.get("license") != "MIT":
...
for marker in ["--ignore-workspace", "--offline", "packedManifest.private===false", "renderToString", "typescript/bin/tsc", "internals=absent", "componentNames.length===82", "PACKED_CAROUSEL_SSR PASS", "PACKED_TIME_RANGE_SSR PASS", "allowedTopLevel", "reproducible=pass", "allowList=pass"]:
...
if distribution_budgets != {"packedFiles": 375, "packedTarballRaw": 410000, "packedUnpackedRaw": 2320000}:
    failures.append("p39:distribution-budgets")
if "Time Range P52" not in components_page or "1.48.0" not in components_page or "V1.48.0" not in preview:
...
for marker in ["RELEASE_CONTRACT PASS", "componentNames.length===82", "actions/upload-artifact@v7", "actions/attest@v4", "artifact-metadata: write", "gh release create"]:
...
if "V1.48.0" not in app or "V1.48.0" not in login_page or "badge:82" not in components_page or "<td>Watermark</td>" not in components_page:
...
if len(visual_baselines) < 25 or any(path.stat().st_size < 1000 for path in visual_baselines):
...
if dist_lib.exists():
    ...
    if len(component_js) != 82 or len(component_types) != 82:
    ...
    if len(style_files) != 82 or not (dist_lib / "styles/core.css").is_file() or not (dist_lib / "styles/manifest.json").is_file():
...
print("- extensions=date-picker,date-range-picker,time-picker,time-range-picker,calendar,image,pagination,upload,layout,float-button")
