...
if any(marker not in preview for marker in ["V1.61.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.61.0" or package.get("private") is not False or package.get("license") != "MIT":
...
if distribution_budgets != {"packedFiles": 402, "packedTarballRaw": 505000, "packedUnpackedRaw": 2900000}:
    failures.append("p39:distribution-budgets")
if "Popover P65" not in components_page or "1.61.0" not in components_page or "V1.61.0" not in preview:
...
if "V1.61.0" not in app or "V1.61.0" not in login_page or "badge:89" not in components_page or "<td>Watermark</td>" not in components_page:
...
if len(visual_baselines) < 38 or any(path.stat().st_size < 1000 for path in visual_baselines):
...
if performance_budgets.get("releaseBaseline", {}).get("enhancementAllowance", {}) != {"packageJsRaw": 40500, "packageJsGzip": 13250, "packageCssRaw": 55100, "packageCssGzip": 9850, "rootCssRaw": 24900, "rootCssGzip": 3610, "subpathConsumerCssRaw": 100, "standaloneExampleJsRaw": 48200, "standaloneExampleCssRaw": 25500, "subpathConsumerJsRaw": 300}:
    failures.append("performance:p65-bounded-component-enhancement-policy")
