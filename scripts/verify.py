...
non_select_vue_sources = "\n".join(path.read_text(encoding="utf-8") for path in vue_source_paths if path.name not in {"UiSelect.vue", "UiMultiSelect.vue"})
if "<select" in non_select_vue_sources or 'class="ui-select-native"' not in (ROOT / "src/components/UiSelect.vue").read_text(encoding="utf-8") or 'class="ui-multi-select-native"' not in (ROOT / "src/components/UiMultiSelect.vue").read_text(encoding="utf-8"):
...
if len(teleport_sources) != 16 or any("useTeleportThemeScope" not in source or 'v-bind="portalThemeAttrs"' not in source for source in teleport_sources.values()):
...
if any(marker not in preview for marker in ["V1.69.0", "previewMotionSwitch", "data-ui-motion-preference"]):
...
if package.get("version") != "1.69.0" or package.get("private") is not False or package.get("license") != "MIT":
...
if distribution_budgets != {"packedFiles": 410, "packedTarballRaw": 580000, "packedUnpackedRaw": 3420000}:
    failures.append("p39:distribution-budgets")
if "MultiSelect P73" not in components_page or "1.69.0" not in components_page or "V1.69.0" not in preview:
...
if "V1.69.0" not in app or "V1.69.0" not in login_page or "badge:91" not in components_page or "<td>Watermark</td>" not in components_page:
...
if len(visual_baselines) < 42 or any(path.stat().st_size < 1000 for path in visual_baselines):
...
if performance_budgets.get("releaseBaseline", {}).get("enhancementAllowance", {}) != {'packageJsRaw': 118000, 'packageJsGzip': 39000, 'packageCssRaw': 156000, 'packageCssGzip': 29000, 'rootCssRaw': 48000, 'rootCssGzip': 8000, 'largestComponentCssRaw': 1000, 'largestComponentCssGzip': 1000, 'subpathConsumerJsRaw': 7250, 'subpathConsumerCssRaw': 1600, 'standaloneExampleJsRaw': 143000, 'standaloneExampleCssRaw': 49000}:
