...
BASELINE = ROOT / ".baseline" / "maturity-p79-v1"
OLD_ARCHIVE = BASELINE / "artifacts" / "lan-ui-v1.zip.orig"
NEW_ARCHIVE = ARTIFACTS / "lan-ui-v1.zip"
PATCH = ARTIFACTS / "maturity-p79-v1.patch"
BASELINE_ARCHIVE = ARTIFACTS / "maturity-p79-v1-baseline.zip"
ROLLBACK = ARTIFACTS / "rollback-maturity-p79-v1.ps1"


def rollback_script(added: list[str], baseline_count: int, artifact_count: int) -> str:
    script = base.rollback_script(added, baseline_count, artifact_count).replace("p11", "p79").replace("P11", "P79")
    script = script.replace(
        "    'rollback-maturity-p79-v1.ps1'",
        "    'rollback-maturity-p79-v1.ps1',\n    'lan-ui-design-system-1.75.0.tgz',\n    'lan-ui-design-system-1.75.0.tgz.sha256'",
    )
    script = script.replace(
...
def main() -> None:
    if not OLD_ARCHIVE.is_file() or not NEW_ARCHIVE.is_file():
        raise FileNotFoundError("P79 baseline and modified package archives are required")
    ...
    with tempfile.TemporaryDirectory(prefix="lan-ui-p79-patch-") as folder:
        repo = Path(folder)
        ...
        base.run_git(git, repo, "commit", "-qm", "P79 baseline")
    ...
    print(f"P79_ARTIFACTS PASS added={len(added)} changed={len(changed)} deleted={len(deleted)} baseline={len(old_files)} artifacts={len(originals)} patch={PATCH.stat().st_size}B")
