...
const multiDemo=ref(['east']);const multiRemoteDemo=ref([]);const multiState=ref('等待选择');const treeDemo=ref(['hangzhou']);const treeRemoteDemo=ref('');const treeState=ref('等待树选择');const cascaderDemo=ref(['zhejiang','hangzhou']);const transferDemo=ref(['api'])
...
const descriptionItems=[{key:'name',label:'本轮能力',value:'TreeSelect P74 · MultiSelect P73 · Select P72 · Selection Controls P71 · Textarea P70 · Input P69 · Button P68 · Collapse P67 · Dropdown P66 · Popover P65 · Tooltip P64 · Breadcrumb P63 · Steps P62 · Timeline P61 · Tag P60 · Card P59'},{key:'version',label:'版本',value:'1.70.0'},{key:'status',label:'状态',value:'稳定'},{key:'owner',label:'负责团队',value:'设计系统组'},{key:'updated',label:'更新日期',value:'2026-08-22'},{key:'coverage',label:'覆盖范围',value:'91 个公开组件 · UiTreeSelect 受控与非受控值/展开状态、字段映射、本地搜索、IME、AbortSignal 懒加载、失败重试与竞态保护、复选级联/严格模式、选择上下限、路径与折叠标签、虚拟化、原生表单、完整树键盘、Portal、命令式 API、ARIA、RTL、SSR、类型、视觉、无障碍、跨浏览器及隔离 tarball 消费回归'}]
...
const qrStatus=ref('expired');const qrRevision=ref(1);const qrValue=computed(()=>`https://lan-ui.example/release/1.70.0?revision=${qrRevision.value}`)
          ...
          <div class="demo-block">
            ...
            <div class="qr-code-showcase" data-qr-code-state-contract="active loading expired scanned invalid refresh download svg ecc icon ssr">
              <div class="qr-code-showcase-primary">
                ...
                <UiQRCode :value="qrValue" :status="qrStatus" level="H" color="#155EEF" :size="184" downloadable download-name="lan-ui-release.svg" label="Lan UI 1.70.0 发布二维码" caption="扫码打开 1.70.0 发布记录" @refresh="refreshQrCode" @download="toast.success('SVG 二维码已下载')"/>
            ...
            <div class="barcode-showcase" data-barcode-state-contract="code128 code39 ean upc itf msi codabar pharmacode auto active loading expired scanned invalid refresh download svg ssr">
              <div class="barcode-showcase-primary">
                ...
                <UiBarcode :value="barcodeValue" :status="barcodeStatus" format="CODE128" color="#155EEF" :width="2" :height="82" downloadable download-name="lan-ui-asset.svg" label="Lan UI 1.70.0 资产条形码" caption="资产标签 · CODE128" @refresh="refreshBarcode" @download="toast.success('SVG 条形码已下载')"/>
