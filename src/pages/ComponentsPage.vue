...
const multiDemo=ref(['east']);const multiRemoteDemo=ref([]);const multiState=ref('等待选择');const treeDemo=ref('hangzhou');const cascaderDemo=ref(['zhejiang','hangzhou']);const transferDemo=ref(['api'])
...
const descriptionItems=[{key:'name',label:'本轮能力',value:'MultiSelect P73 · Select P72 · Selection Controls P71 · Textarea P70 · Input P69 · Button P68 · Collapse P67 · Dropdown P66 · Popover P65 · Tooltip P64 · Breadcrumb P63 · Steps P62 · Timeline P61 · Tag P60 · Card P59'},{key:'version',label:'版本',value:'1.69.0'},{key:'status',label:'状态',value:'稳定'},{key:'owner',label:'负责团队',value:'设计系统组'},{key:'updated',label:'更新日期',value:'2026-08-22'},{key:'coverage',label:'覆盖范围',value:'91 个公开组件 · UiMultiSelect 受控与非受控数组、字段映射、本地与远程搜索、选择上下限、全选、标签折叠、原生多值表单、键盘与 Backspace、缓存与竞态保护、命令式 API、ARIA、RTL、SSR、类型、视觉、无障碍、跨浏览器及隔离 tarball 消费回归'}]
...
const qrStatus=ref('expired');const qrRevision=ref(1);const qrValue=computed(()=>`https://lan-ui.example/release/1.69.0?revision=${qrRevision.value}`)
function refreshQrCode(){qrRevision.value+=1;qrStatus.value='active';toast.success('二维码已刷新')}
const barcodeStatus=ref('expired');const barcodeRevision=ref(1);const barcodeValue=computed(()=>`LAN-UI-169-R${barcodeRevision.value}`)
...
&lt;UiButton href="/docs" target="_blank" icon-position="end"&gt;文档&lt;/UiButton&gt;</code></pre>
        ...
        <section id="forms" class="card doc-section">
          ...
          <div class="demo-block form-showcase">
            ...
            <div class="form-demo-section" data-multi-select-state-contract="array search limits tags clear keyboard form aria rtl"><div class="form-demo-title"><strong>高级选择</strong><span>MultiSelect P73 / TreeSelect / Cascader</span></div><div class="form-demo-content"><div class="advanced-control-grid"><UiFormItem label="生产级多选" required help="字段映射、全选、上限、标签折叠与原生表单"><UiMultiSelect v-model="multiDemo" :options="advancedOptions" :field-names="{label:'name',value:'id',description:'detail',keywords:'terms',disabled:'locked'}" searchable clearable show-select-all :max-count="3" :max-tag-count="2" name="regions" required @change="(values,meta)=>multiState=`${meta.source} · ${values.join(', ')}`"><template #prefix>◇</template><template #footer="{options}">{{ options.length }} 个可见选项 · 最多选择 3 项</template></UiMultiSelect></UiFormItem><UiFormItem label="远程多选" help="防抖、AbortSignal、竞态保护、缓存与重试"><UiMultiSelect v-model="multiRemoteDemo" searchable clearable :remote-method="fetchMultiDemo" :remote-debounce="180" :remote-min-chars="1" placeholder="搜索发布组" @change="(values,meta)=>multiState=`remote ${meta.source} · ${values.join(', ')}`"/></UiFormItem><UiFormItem label="树选择"><UiTreeSelect v-model="treeDemo" :options="treeOptions"/></UiFormItem><UiFormItem label="级联选择"><UiCascader v-model="cascaderDemo" :options="cascaderOptions"/></UiFormItem></div><div class="preview-note"><strong>多选状态：</strong>{{ multiState }}。支持 Arrow / Home / End / Enter / Escape / Tab、非搜索 Typeahead、空查询 Backspace 删除，以及 Loading / Error / Empty / Readonly / Disabled。</div></div></div>
          ...
          <div class="demo-block">
            ...
            <div class="qr-code-showcase" data-qr-code-state-contract="active loading expired scanned invalid refresh download svg ecc icon ssr">
              <div class="qr-code-showcase-primary">
                ...
                <UiQRCode :value="qrValue" :status="qrStatus" level="H" color="#155EEF" :size="184" downloadable download-name="lan-ui-release.svg" label="Lan UI 1.69.0 发布二维码" caption="扫码打开 1.69.0 发布记录" @refresh="refreshQrCode" @download="toast.success('SVG 二维码已下载')"/>
            ...
            <div class="barcode-showcase" data-barcode-state-contract="code128 code39 ean upc itf msi codabar pharmacode auto active loading expired scanned invalid refresh download svg ssr">
              <div class="barcode-showcase-primary">
                ...
                <UiBarcode :value="barcodeValue" :status="barcodeStatus" format="CODE128" color="#155EEF" :width="2" :height="82" downloadable download-name="lan-ui-asset.svg" label="Lan UI 1.69.0 资产条形码" caption="资产标签 · CODE128" @refresh="refreshBarcode" @download="toast.success('SVG 条形码已下载')"/>
