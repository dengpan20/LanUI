...
const descriptionItems=[{key:'name',label:'组件名称',value:'UiCalendar'},{key:'version',label:'当前版本',value:'1.20.0'},{key:'status',label:'状态',value:'稳定'},{key:'owner',label:'维护团队',value:'数据录入组'},{key:'updated',label:'最近更新',value:'2026-08-12'},{key:'coverage',label:'用例覆盖',value:'单选、多选、范围、年份、键盘、时区、禁用、RTL、ARIA'}]
...
import UiButton from 'lan-ui-design-system/components/UiButton'</code></pre>
        ...
        <section id="states" class="card doc-section"><header class="doc-section-header"><h2>交互状态矩阵</h2><p>每个组件在交付前都需覆盖以下状态和键盘行为。</p></header><div class="demo-block" style="overflow:auto"><table class="state-matrix"><thead><tr><th>组件</th><th>Default</th><th>Hover</th><th>Pressed</th><th>Focus</th><th>Disabled</th><th>Loading / Error</th></tr></thead><tbody><tr><td>Button</td><td>基础视觉</td><td>加深 + 上移</td><td>复位</td><td>3px Ring</td><td>48% 透明</td><td>Spinner</td></tr><tr><td>Input</td><td>轻边框</td><td>品牌浅边框</td><td>—</td><td>品牌边框 + Ring</td><td>弱背景</td><td>红边框 + 文案</td></tr><tr><td>Tree supports Arrow, Home/End, *, Enter, Space and typeahead; AutoComplete</td><td>自由输入</td><td>候选高亮</td><td>选择并提交</td><td>Combobox + Active descendant</td><td>只读 / 禁用</td><td>异步加载 / 空 / 错误</td></tr><tr><td>NumberInput</td><td>数值草稿</td><td>控制键高亮</td><td>步进并限界</td><td>Spinbutton + Ring</td><td>控制键锁定</td><td>解析错误 + 恢复</td></tr><tr><td>Slider / Range</td><td>单值 / 区间</td><td>Tooltip + 高亮</td><td>拖拽 / 点击 Mark</td><td>ARIA slider + Ring</td><td>禁止交互</td><td>只读 / 错误</td></tr><tr><td>Rate</td><td>integer / fractional</td><td>live preview</td><td>select / clear</td><td>ARIA slider + Ring</td><td>readonly / disabled</td><td>form error</td></tr><tr><td>Calendar</td><td>single / multiple / range</td><td>range preview</td><td>select / clear / today</td><td>ARIA grid + roving tabindex</td><td>readonly / disabled date</td><td>min / max / empty</td></tr><tr><td>Tree</td><td>Hierarchy</td><td>Active node</td><td>Select / Check</td><td>Active descendant + Ring</td><td>Node / Tree disabled</td><td>Lazy / Retry / Empty</td></tr><tr><td>Navigation</td><td>次级文字</td><td>弱白背景</td><td>加深</td><td>可见 Ring</td><td>不可点击</td><td>展开/收起</td></tr><tr><td>Table row</td><td>白色表面</td><td>品牌浅背景</td><td>—</td><td>选择框焦点</td><td>—</td><td>Skeleton / Empty</td></tr></tbody></table><div class="preview-note"><strong>键盘：</strong> Tab 遍历控件；Enter/Space 激活；AutoComplete、数值框和滑块支持方向键；数值框和滑块额外支持 Page、Home、End；Esc 关闭浮层；Modal 打开后焦点进入弹层，关闭后返回触发元素。</div></div></section>
      </main>
    </div>
  </div>
</template>
