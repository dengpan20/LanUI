...
const menuItems=[{key:'overview',label:'项目总览',icon:'home'},{key:'resources',label:'资源管理',icon:'layers',children:[{key:'components',label:'组件清单',badge:76},{key:'tokens',label:'设计 Token'}]},{key:'disabled',label:'已停用入口',icon:'file',disabled:true}]
...
const descriptionItems=[{key:'name',label:'本轮能力',value:'Data List P46'},{key:'version',label:'版本',value:'1.42.0'},{key:'status',label:'状态',value:'稳定'},{key:'owner',label:'负责团队',value:'设计系统组'},{key:'updated',label:'更新日期',value:'2026-08-14'},{key:'coverage',label:'覆盖范围',value:'76 个公开组件 · API 清单、独立样式、SSR、RTL、ARIA、响应式网格、Node 20/22/24 与隔离 tarball 消费回归'}]
  ...
  @select="runCommand" /&gt;</code></pre>
              ...
              <div class="watermark-showcase-card" style="grid-column:1/-1">
                ...
                <UiWatermark :content="['Lan UI','内部审阅']" :gap="[72,64]" :font="{fontSize:14,color:'rgba(37,99,235,.16)',fontWeight:650}" aria-label="内部审阅文档水印">
                  <div class="watermark-demo-document"><div><strong>组件发布检查单</strong><span>Release 1.42.0 · 视觉、键盘、类型、SSR 与产物验证</span></div><UiTag color="green">Ready</UiTag></div>
                </UiWatermark>
