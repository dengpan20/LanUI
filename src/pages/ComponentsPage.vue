...
const demoDate=ref('2026-08-11');const calendarDemo=ref('2026-08-12');const calendarRangeDemo=ref(['2026-08-10','2026-08-16']);const demoTime=ref('09:30');const demoDateTime=ref('2026-08-11T14:30');const zonedInstant=ref(new Date('2026-08-11T06:30:00.000Z'));const demoTimeZone=ref('Asia/Shanghai');const demoFiles=ref([])
...
const formRef=ref(null);const validatedForm=reactive({customer:{name:'',email:''}});const validationRules={customer:{name:[{required:true,message:'请输入客户名称'},{min:2,message:'客户名称至少 2 个字符'}],email:[{required:true,message:'请输入企业邮箱'},{type:'email',message:'请输入有效的企业邮箱'}]}}
...
const descriptionItems=[{key:'name',label:'当前成熟度',value:'Managed Form P28'},{key:'version',label:'当前版本',value:'1.24.0'},{key:'status',label:'状态',value:'稳定'},{key:'owner',label:'维护团队',value:'基础组件组'},{key:'updated',label:'最近更新',value:'2026-08-13'},{key:'coverage',label:'用例覆盖',value:'嵌套路径、异步竞态、服务端错误、局部重置、错误摘要、SSR、RTL、ARIA'}]
...
&lt;/UiButton&gt;</code></pre>
        ...
        <section id="forms" class="card doc-section">
          ...
          <div class="demo-block form-showcase">
            ...
            <div class="form-demo-section"><div class="form-demo-title"><strong>表单校验</strong><span>嵌套字段 / 异步竞态 / 错误摘要 / 服务端错误</span></div><div class="form-demo-content"><UiForm ref="formRef" :model="validatedForm" :rules="validationRules" show-error-summary focus-on-error @submit="emit('notify','表单校验通过并已提交')" @invalid="emit('notify','请修正表单错误','error')"><template #default="{dirty,validating,errors}"><div class="form-row"><UiFormItem name="customer.name" label="客户名称" required show-success><UiInput v-model="validatedForm.customer.name" clearable placeholder="至少输入 2 个字符"/></UiFormItem><UiFormItem name="customer.email" label="企业邮箱" required show-success><UiInput v-model="validatedForm.customer.email" clearable placeholder="name@company.com"/></UiFormItem></div><div class="ui-form-actions"><span class="field-help">{{ dirty?'表单已有修改':'尚未修改' }} · {{ errors.length }} 个错误</span><UiButton type="button" variant="text" @click="formRef.setFieldError('customer.email','该邮箱已被其他账户使用')">模拟服务端错误</UiButton><UiButton type="button" variant="text" @click="formRef.resetFields('customer.email')">重置邮箱</UiButton><UiButton type="reset" variant="secondary">全部重置</UiButton><UiButton type="submit" :loading="validating">提交校验</UiButton></div></template></UiForm></div></div>
