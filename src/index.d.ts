...
export interface UiFormListChange<Item=unknown> { type:'add'|'remove'|'move'|'replace'; values:Item[]; index?:number; value?:Item; indices?:number[]; removed?:Item[]; from?:number; to?:number }
...
export type UiSchemaFormBuiltinType = 'input'|'textarea'|'number'|'select'|'autocomplete'|'checkbox'|'switch'|'date'|'time'|'date-range'|'slider'|'segmented'
export interface UiSchemaFormContext<Model extends Record<string,unknown>=Record<string,unknown>> { model:Model; field:UiSchemaFormField<Model>|UiSchemaFormSection<Model>; section?:UiSchemaFormSection<Model>; getFieldValue:(name:UiFormPath)=>unknown; getFieldsValue:()=>Model }
...
export interface UiSchemaFormSection<Model extends Record<string,unknown>=Record<string,unknown>> { key?:string; title?:string; description?:string; titleTag?:'h2'|'h3'|'h4'; columns?:number; class?:string|string[]|Record<string,boolean>; visible?:UiSchemaFormResolver<boolean,Model>; fields?:UiSchemaFormField<Model>[]; children?:UiSchemaFormField<Model>[] }
export type UiSchemaFormNode<Model extends Record<string,unknown>=Record<string,unknown>> = UiSchemaFormField<Model>|UiSchemaFormSection<Model>
export interface UiSchemaFormFieldChange<Model extends Record<string,unknown>=Record<string,unknown>> { name:string; value:unknown; previous:unknown; field:UiSchemaFormField<Model>; model:Model }
...
export interface UiSchemaFormInstance<Model extends Record<string,unknown>=Record<string,unknown>> extends UiFormInstance<Model> { getFieldDefinition:(name:UiFormPath)=>UiSchemaFormField<Model>|null; getVisibleFields:()=>UiSchemaFormField<Model>[] }
