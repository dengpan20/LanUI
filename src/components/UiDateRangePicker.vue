...
const placeholderKey=computed(()=>props.mode==='time'?'date.timePlaceholder':props.mode==='datetime'?'date.datetimePlaceholder':'date.placeholder')
...
<template><span class="ui-date-range-picker" :class="[attrs.class,`size-${resolvedSize}`,{focused,invalid:resolvedInvalid,disabled,'has-value':values[0]||values[1]}]" :style="attrs.style" role="group" :aria-labelledby="labelledby" :aria-label="labelledby?undefined:(attrs['aria-label']||t('date.rangeLabel'))" :aria-describedby="describedby" :aria-invalid="resolvedInvalid||undefined" :data-time-zone="timeZone" :data-value-type="resolvedValueType">
  <AppIcon :name="mode==='time'?'clock':'calendar'" :size="15" class="ui-date-icon"/>
  <input :id="controlId" ref="startInput" class="ui-date-range-native" :type="inputType" :value="values[0]" :placeholder="startPlaceholder||t('date.start')" :aria-label="startPlaceholder||t('date.start')" :min="nativeMin" :max="constrain?(values[1]||nativeMax):nativeMax" :step="step" :disabled="disabled" @input="update(0,$event)" @focus="onFocus(0,$event)" @blur="onBlur(0,$event)"/>
  <span class="ui-date-range-separator" aria-hidden="true">{{ separator||t('date.separator') }}</span>
  <input ref="endInput" class="ui-date-range-native" :type="inputType" :value="values[1]" :placeholder="endPlaceholder||t('date.end')" :aria-label="endPlaceholder||t('date.end')" :min="constrain?(values[0]||nativeMin):nativeMin" :max="nativeMax" :step="step" :disabled="disabled" @input="update(1,$event)" @focus="onFocus(1,$event)" @blur="onBlur(1,$event)"/>
