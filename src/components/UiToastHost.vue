...
<template><Teleport to="body"><div v-for="placement in placements" v-bind="portalThemeAttrs" :key="placement" class="toasts" :class="`toasts-${placement}`" :dir="direction" :style="[portalThemeStyle,{zIndex:toastZIndex}]" aria-live="polite"><TransitionGroup name="toast-list"><div v-for="item in source.filter(entry=>entry.placement===placement)" :key="item.id" class="toast" :class="[item.type,{out:item.out}]" :role="item.type==='error'?'alert':'status'" @mouseenter="pause(item.id);$emit('pause',item.id)" @mouseleave="resume(item.id);$emit('resume',item.id)"><span class="toast-icon"><AppIcon :name="icon(item.type)" :size="14"/></span><div class="toast-copy"><strong>{{ item.title||title(item.type) }}</strong><p>{{ item.message }}</p></div><button class="icon-btn" style="width:24px;height:24px" :aria-label="t('toast.close')" @click="remove(item.id);$emit('remove',item.id)"><AppIcon name="close" :size="13"/></button></div></TransitionGroup></div></Teleport></template>
