...
const resolvedItemHeight=computed(()=>Math.max(24,Number(props.itemHeight)||32))
function optionItemHeight(option){
  return option?.description?Math.max(44,resolvedItemHeight.value):resolvedItemHeight.value
}
...
<template>
  <div v-bind="passthroughAttrs" :id="controlId" ref="rootRef" class="ui-transfer" :class="[`size-${resolvedSize}`,attrs.class,{disabled,readonly,invalid:resolvedInvalid,loading:busy,error:failed,'one-way':oneWay,'has-operation-text':operations.length}]" :style="attrs.style" role="group" :aria-label="resolvedAriaLabel" :aria-labelledby="labelledby" :aria-describedby="describedby" :aria-disabled="disabled||undefined" :aria-readonly="readonly||undefined" :aria-invalid="resolvedInvalid||undefined" :aria-required="resolvedRequired||undefined" :aria-busy="busy||undefined" data-ui-transfer :data-state="state" :data-direction="direction" @focusin="handleFocusIn" @focusout="handleFocusOut">
    ...
    <section class="ui-transfer-panel" data-direction="left" :aria-label="resolvedTitles[0]">
      ...
      <UiVirtualList ref="leftListRef" class="ui-transfer-list" :items="leftRecords" item-key="value" text-field="label" selection-mode="multiple" :model-value="leftSelected" :active-index="activeIndexes.left" :disabled-keys="leftRecords.filter(optionDisabled).map(option=>option.value)" :item-size="optionItemHeight" :estimated-item-size="resolvedItemHeight" :height="listHeight" :overscan="overscan" :measure="measure" :loading="panelLoading('left')" :error="rawPanelError('left')" :aria-label="resolvedTitles[0]" :tabindex="disabled?-1:0" @update:model-value="onPanelSelection('left',$event)" @update:active-index="activeIndexes={...activeIndexes,left:$event}" @keydown.capture="onListKeydown('left',$event)">
    ...
    <section class="ui-transfer-panel" data-direction="right" :aria-label="resolvedTitles[1]">
      ...
      <UiVirtualList ref="rightListRef" class="ui-transfer-list" :items="rightRecords" item-key="value" text-field="label" selection-mode="multiple" :model-value="rightSelected" :active-index="activeIndexes.right" :disabled-keys="rightRecords.filter(optionDisabled).map(option=>option.value)" :item-size="optionItemHeight" :estimated-item-size="resolvedItemHeight" :height="listHeight" :overscan="overscan" :measure="measure" :loading="panelLoading('right')" :error="rawPanelError('right')" :aria-label="resolvedTitles[1]" :tabindex="disabled?-1:0" @update:model-value="onPanelSelection('right',$event)" @update:active-index="activeIndexes={...activeIndexes,right:$event}" @keydown.capture="onListKeydown('right',$event)">
